...
adata_input.layers['raw_spliced'] = adata[adata_input.obs_names, adata_input.var_names].layers['spliced']
adata_input.layers['raw_unspliced'] = adata[adata_input.obs_names, adata_input.var_names].layers['unspliced']
...
adata_model_pos_split = train_model(adata_input, max_epochs=4000, svi_train=False, log_every=100,
                                    patient_init=45, batch_size=-1, use_gpu=0, cell_state='state_info',
                                    include_prior=True, offset=False, library_size=True,
                                    model_type='auto', 
                                    patient_improve=1e-4, 
                                    guide_type='auto_t0_constraint', train_size=1.0)
...
adata_cospar_sub = adata_cospar[adata_input.obs_names.str.replace('-\d-\d', ''), :].copy()

adata_cytotrace_sub = adata_cytotrace[adata_input.obs_names.str.replace('-\d-\d', ''), :].copy()
...
def check_shared_time(adata_model_pos, adata):
    ...
    scv.pl.scatter(adata, color='latent_time', show=False,
                   ax=ax[0], title='scvelo %.2f' % spearmanr(1-adata_cytotrace_sub.obs.cytotrace, adata.obs.latent_time)[0],
    ...
    scv.pl.scatter(adata_cospar_sub, color='fate_potency', show=False, ax=ax[3], basis='emb')
    ...
    fig.savefig("fig3_bifurcation_test_sub_model1.pdf", facecolor=fig.get_facecolor(), bbox_inches='tight', edgecolor='none', dpi=300)
#velocity_samples = pos['beta'] * pos['ut'] / scale - pos['gamma'] * pos['st']
...
v_map_all, embeds_radian, fdri = vector_field_uncertainty(adata_input, adata_model_pos_split[1], basis='emb', denoised=False, n_jobs=1)

fig, ax = plt.subplots()
embed_mean = plot_mean_vector_field(adata_model_pos_split[1], adata_input, ax=ax, basis='emb')
fig.savefig('fig3_uni_bifurcation_vectorfield_model1.pdf')

adata_input.write("fig3_larry_uni_bifurcation_top2000_model1.h5ad")
adata_model_pos_split[0].save('Fig3_uni_bifurcation_model1', overwrite=True)
...
with open("fig3_uni_bifurcation_data_model1.pkl", "wb") as f:
