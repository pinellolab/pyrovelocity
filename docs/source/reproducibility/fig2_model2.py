...
with open("fig2_pbmc_data_model2.pkl", "rb") as f:
...
adata_pbmc = scv.read("fig2_pbmc_processed_model2.h5ad")
...
ress = pd.DataFrame({"cell_type": adata_pbmc.obs['celltype'].values,
...
scv.pl.velocity_embedding_stream(adata_pbmc, basis='tsne', fontsize=font_size, ax=ax[1], title='', **kwargs)
ax[1].set_title("Scvelo\n", fontsize=7)

scv.pl.velocity_embedding_stream(adata_pbmc, fontsize=font_size, basis='tsne',
...
plot_arrow_examples(adata_pbmc, np.transpose(v_map_all_pbmc, (1, 2, 0)), embeds_radian_pbmc, ax=ax[3],
                    n_sample=30, fig=pbmcfig_A0, basis='tsne', scale=None, alpha=0.3, index=100,
                    scale2=None, num_certain=0, num_total=4)
ax[3].set_title("Single cell\nvector field", fontsize=7)

plot_vector_field_uncertain(adata_pbmc, embed_mean_pbmc, embeds_radian_pbmc, fig=pbmcfig_A0[1], cbar=True,
                            basis='tsne', scale=0.018, arrow_size=5)

pbmcfig_A0[0].subplots_adjust(hspace=0.2, wspace=0.1, left=0.01, right=0.99, top=0.99, bottom=0.45)
pbmcfig_A0[1].subplots_adjust(hspace=0.2, wspace=0.1, left=0.01, right=0.99, top=0.99, bottom=0.45)
...
ax[0].text(-0.1, 1.18, 'b', transform=ax[0].transAxes,
           fontsize=7, fontweight='bold', va='top', ha='right')
kwargs = dict(color='gray', density=.8, add_margin=.1, s=dot_size,
              show=False, alpha=.2, min_mass=3.5, frameon=False)
...
plot_arrow_examples(adata, np.transpose(v_map_all, (1, 2, 0)), embeds_radian, ax=ax[3],
                    n_sample=30, fig=fig, basis='umap', scale=0.005, alpha=0.2, index=5,
                    scale2=0.03, num_certain=6, num_total=6)
ax[3].set_title("Single cell\nvector field", fontsize=7)

plot_vector_field_uncertain(adata, embed_mean, embeds_radian, fig=subfig_A0[1], cbar=True, basis='umap', 
                            scale=0.03,
                            arrow_size=5)
subfig_A0[0].subplots_adjust(hspace=0.2, wspace=0.1, left=0.01, right=0.99, top=0.80, bottom=0.33)
subfig_A0[1].subplots_adjust(hspace=0.2, wspace=0.1, left=0.01, right=0.99, top=0.80, bottom=0.33)
...
volcano_data2, _ = plot_gene_ranking([adata_model_pos], [adata], ax=ax[1],
...
_ = rainbowplot(volcano_data2, adata, adata_model_pos, subfig_B[1], data=['st', 'ut'], num_genes=4)

fig.savefig("Fig2_pancreas_raw_gene_selection_model2.tif", facecolor=fig.get_facecolor(), bbox_inches='tight', edgecolor='none', dpi=300)

fig.savefig("Fig2_pancreas_raw_gene_selection_model2.svg", facecolor=fig.get_facecolor(), bbox_inches='tight', edgecolor='none', dpi=300)
...
# supfig.savefig("SuppFig1_model1.tif", facecolor=fig.get_facecolor(), bbox_inches='tight', edgecolor='none', dpi=300)
supfig.savefig("SuppFig1_model2.tif.svg", facecolor=supfig.get_facecolor(), bbox_inches='tight', edgecolor='none', dpi=300)
