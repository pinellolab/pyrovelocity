...
ax[0].text(-0.22, 1.15, 'a', transform=ax[0].transAxes,
...
sns.scatterplot(x='X1', y='X2', data=ress, alpha=0.9, s=s,
                linewidth=0, edgecolor="none", hue='cell_type',
                ax=ax[0], legend='brief')
ax[0].legend(bbox_to_anchor=[2.9, -0.01], ncol=4, prop={'size': 7}, fontsize=7, frameon=False)
ax[0].axis('off')
scv.pl.velocity_embedding_grid(adata, color='clusters', ax=ax[0],
                               show=False, legend_loc='none', scale=0.3, density=0.6, arrow_size=3, s=s, alpha=0)
...
scv.pl.velocity_embedding_grid(adata_sub, color='clusters', ax=ax[1], show=False, legend_loc='none', scale=0.3, density=0.4, arrow_size=3, s=s)
...
scv.tl.velocity_embedding(adata)
adata_sub = adata[adata.obs.clusters=='Epsilon']
scv.pl.velocity_embedding_grid(adata_sub, color='clusters', ax=ax[2], show=False, legend_loc='none', scale=0.3, density=0.4, arrow_size=3, s=s)
...
scv.tl.velocity_embedding(adata)
adata_sub = adata[adata.obs.clusters=='Epsilon']
scv.pl.velocity_embedding_grid(adata_sub, color='clusters', ax=ax[3], show=False, legend_loc='none', scale=0.3, density=0.4, arrow_size=3, s=s)
...
scv.tl.velocity_embedding(adata)
adata_sub = adata[adata.obs.clusters=='Epsilon']
scv.pl.velocity_embedding_grid(adata_sub, color='clusters', ax=ax[4], show=False, legend_loc='none', scale=0.3, density=0.4, arrow_size=3, s=s)
ax[4].set_title("30 neighbors\n3000 variable genes", fontsize=7)

bin = 30
adata_pbmc.obs['shared_time_uncertain'] = adata_model_pos_pbmc['cell_time'].std(0).flatten()
scv.pl.scatter(adata_pbmc, c='shared_time_uncertain', ax=ax[5], colorbar=True, 
               basis='tsne', show=False, cmap='inferno', fontsize=7)
ax[5].text(-0.18, 1.15, 'b', transform=ax[5].transAxes,
           fontsize=7, fontweight='bold', va='top', ha='right')
select = adata_pbmc.obs['shared_time_uncertain'] > np.quantile(adata_pbmc.obs['shared_time_uncertain'], 0.9)
sns.kdeplot(adata_pbmc.obsm['X_tsne'][:, 0][select],
            adata_pbmc.obsm['X_tsne'][:, 1][select],
            ax=ax[5], levels=3, fill=False)
...
scv.pl.scatter(adata_pbmc, c='vector_field_rayleigh_test',
               basis='tsne', ax=ax[6], show=False, cmap='inferno_r', fontsize=7, colorbar=True)
select = adata_pbmc.obs['vector_field_rayleigh_test'] > np.quantile(adata_pbmc.obs['vector_field_rayleigh_test'], 0.9)
sns.kdeplot(adata_pbmc.obsm['X_tsne'][:, 0][select],
            adata_pbmc.obsm['X_tsne'][:, 1][select], ax=ax[6], levels=3, fill=False)
ax[6].set_title("Vector field rayleigh test <5%% FDR:%.2f%%" % (round((fdri_pbmc < 0.05).sum()/fdri_pbmc.shape[0], 3)*100), fontsize=5)

plot_state_uncertainty(adata_model_pos_pbmc, adata_pbmc,
                       basis='tsne', kde=True, data='raw', top_percentile=0.9, ax=ax[7])
...
ax_cb = scv.pl.scatter(adata, c='1-cytotrace', ax=ax[0], show=False, cmap='inferno', fontsize=7, colorbar=True)
ax_cb = scv.pl.scatter(adata, c='latent_time', ax=ax[1], show=False, cmap='inferno', fontsize=7, colorbar=True)
ax_cb = scv.pl.scatter(adata, c='shared_time', ax=ax[2], show=False, cmap='inferno', fontsize=7, colorbar=True)
# bin=30
...
set1 = set(volcano_data2.sort_values("mean_mae", ascending=False).head(300).sort_values("time_correlation", ascending=False).head(50).index)
...
set3 = set(scvelo_top.sort_values("likelihood", ascending=False).head(300).sort_values("cor", ascending=False).head(50).index)
...
ax_cb = scv.pl.scatter(adata, c='shared_time_uncertain', ax=ax[4], show=False, cmap='inferno', fontsize=7, colorbar=True)
...
sns.kdeplot(adata.obsm['X_umap'][:, 0][select],
            adata.obsm['X_umap'][:, 1][select], ax=ax[4], levels=3, fill=False)
...
ax_cb = scv.pl.scatter(adata, c='vector_field_rayleigh_test', ax=ax[5], show=False, cmap='inferno_r', fontsize=7, colorbar=True)
select = adata.obs['vector_field_rayleigh_test'] > np.quantile(adata.obs['vector_field_rayleigh_test'], 0.9)
sns.kdeplot(adata.obsm['X_umap'][:, 0][select],
            adata.obsm['X_umap'][:, 1][select], ax=ax[5], levels=3, fill=False)
ax[5].set_title("Rayleigh test <5%% FDR:%s%%" % (round((fdri < 0.05).sum()/fdri.shape[0], 2)*100), fontsize=5)
...
_ = plot_state_uncertainty(pos, adata, kde=True, data='raw', top_percentile=0.9, ax=ax[6])
...
subfig[0].subplots_adjust(hspace=0.3, wspace=0.18, left=0.0, right=0.92, top=0.92, bottom=0.25)
subfig[1].subplots_adjust(hspace=0.3, wspace=0.15, left=0.0, right=0.92, top=0.92, bottom=0.25)
...
supfigs[0].subplots_adjust(hspace=0.3, wspace=0.1, left=0.01, right=0.75, top=0.92, bottom=0.15)
...
fig.savefig("SuppFigure2.pdf", facecolor=fig.get_facecolor(), bbox_inches='tight', edgecolor='none', dpi=300)
