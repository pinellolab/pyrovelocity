...
def convert_model_state(
    ...
    if "dynamics_output" in model_state and model_state["dynamics_output"] is not None:
    ...
    observations = None
    if "observations" in model_state and model_state["observations"] is not None:
        observations = {}
...
def convert_pyro_to_numpyro_guide(
    ...
    if not callable(pyro_guide) and not hasattr(pyro_guide, '__call__'):
        raise TypeError("Expected a callable Pyro guide function or AutoGuide instance")
