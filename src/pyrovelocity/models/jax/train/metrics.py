...
@beartype
def compute_loss(svi: SVI, params: PyTree, *args, **kwargs) -> float:
    """Compute loss.
...
@beartype
def compute_elbo(
    model: Callable, guide: Callable, params: PyTree, *args, **kwargs
) -> float:
    """Compute ELBO.
...
def compute_predictive_log_likelihood(
    model: Callable, posterior_samples: Dict[str, jnp.ndarray], *args, **kwargs
) -> jnp.ndarray:
    ...
    if "_RETURN" in predictions:
        ...
        log_likelihoods = jnp.zeros(
            (len(next(iter(posterior_samples.values()))), 1)
        )
...
def compute_metrics(
    ...
    **kwargs,
    ...
    pred_ll = compute_predictive_log_likelihood(
        model, posterior_samples, *args, **kwargs
    )
    ...
    metrics["kl_divergence"] = (
        metrics["elbo"] - metrics["predictive_log_likelihood"]
    )
...
def compute_validation_metrics(
    ...
    posterior_samples = {
        "alpha": jnp.ones((num_samples,)) * 2.0
        + 0.1 * jax.random.normal(key, (num_samples,)),
        "beta": jnp.ones((num_samples,)) * 1.0
        + 0.1 * jax.random.normal(jax.random.split(key)[0], (num_samples,)),
    }

    # Compute predictive log likelihood on training data
    train_pred_ll = compute_predictive_log_likelihood(
        model, posterior_samples, **train_data
    )
    metrics["train_predictive_log_likelihood"] = float(jnp.mean(train_pred_ll))

    # Compute predictive log likelihood on validation data
    val_pred_ll = compute_predictive_log_likelihood(
        model, posterior_samples, **val_data
    )
