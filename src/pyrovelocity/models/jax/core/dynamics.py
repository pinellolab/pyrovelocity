...
def standard_dynamics_model(
    ...
    st_special = s0 * expu + alpha / beta * (1 - expu) - (alpha - beta * u0) * tau * expu
...
def nonlinear_dynamics_model(
    ...
    def get_state_at_tau(y0, tau_val):
        ...
        (final_state, _), _ = jax.lax.scan(step_fn, (y0, 0.0), None, length=max_steps)
        return final_state
...
def vectorized_standard_dynamics_model(
    ...
    return vmap_fn(tau, u0, s0, params["alpha"], params["beta"], params["gamma"])
