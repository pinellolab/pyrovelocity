...
def validate_guide_factory_function(fn: Callable) -> bool:
    ...
    if len(params) != 2:
        raise TypeError(
            f"Guide factory function must have 2 parameters, got {len(params)}"
        )
    ...
    if return_annotation == inspect.Signature.empty:
        raise TypeError(
            "Guide factory function must have a return type annotation"
        )

    # Check if the function is bearable as a GuideFactoryFunction
    if not is_bearable(fn, GuideFactoryFunction):
        raise TypeError(
            "Function does not conform to GuideFactoryFunction interface"
        )
