...
def validate_observation_function(fn: Callable) -> bool:
    ...
    if len(params) != 3:
        raise TypeError(
            f"Observation function must have 3 parameters, got {len(params)}"
        )
    ...
    if return_annotation == inspect.Signature.empty:
        raise TypeError(
            "Observation function must have a return type annotation"
        )

    # Check if the function is bearable as an ObservationFunction
    if not is_bearable(fn, ObservationFunction):
        raise TypeError(
            "Function does not conform to ObservationFunction interface"
        )
