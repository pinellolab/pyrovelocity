...
def create_likelihood_function(
    ...
    if fn is None:
        raise ValueError(f"Likelihood function '{config.name}' is not registered")
...
def create_observation_function(
    ...
    if fn is None:
        raise ValueError(f"Observation function '{config.name}' is not registered")
...
def create_guide_factory_function(
    ...
    if fn is None:
        raise ValueError(f"Guide factory function '{config.name}' is not registered")
...
def create_model(
    ...
    def model(
        ...
        with numpyro.plate("gene", n_genes):
            ...
            alpha = numpyro.sample("alpha", numpyro.distributions.Delta(params["alpha"]))
            beta = numpyro.sample("beta", numpyro.distributions.Delta(params["beta"]))
            gamma = numpyro.sample("gamma", numpyro.distributions.Delta(params["gamma"]))
        ...
        likelihood_fn(u_transformed, s_transformed, u_expected, s_expected, scaling_params)
