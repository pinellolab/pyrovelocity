...
class PyroVelocityModel:
    ...
    def get_velocity(
        ...
        **kwargs
        ...
        This method computes RNA velocity from posterior samples generated by the model,
        following the same approach as the legacy implementation's compute_mean_vector_field.
        It first generates posterior samples, then computes velocity, and finally
        computes the velocity embedding using scvelo.
        ...
        Returns:
            Numpy array of velocity values with shape [n_cells, n_genes]
        ...
        Examples:
            ...
            >>> assert velocity.shape == (n_cells, n_genes)
        """
        # Import compute_velocity function
        import scanpy as sc
    ...
    def get_velocity_uncertainty(
        ...
        **kwargs
        ...
        This method computes uncertainty in RNA velocity from posterior samples
        generated by the model. It first generates posterior samples if they are
        not provided, then computes velocity and uncertainty using the compute_velocity
        and compute_uncertainty functions from the inference.posterior module.
        ...
        Returns:
            Numpy array of uncertainty values with shape [n_genes]

        Examples:
            >>> # Compute velocity uncertainty from a trained model
            >>> import anndata as ad
            >>> import numpy as np
            >>> from pyrovelocity.models.modular.factory import create_standard_model
            >>>
            ...
            >>> model.train(adata=adata, max_epochs=2)  # Use small number for testing
            >>>
            >>> # Compute velocity uncertainty
            >>> uncertainty = model.get_velocity_uncertainty(adata=adata)
            >>> assert uncertainty.shape == (n_genes,)
        """
        # Import compute_velocity and compute_uncertainty functions
        from pyrovelocity.models.modular.inference.posterior import (
            compute_uncertainty,
            compute_velocity,
        )
        ...
        velocity_samples = velocity_results["velocity"]
        if isinstance(velocity_samples, np.ndarray):
            velocity_samples = torch.tensor(velocity_samples)
        uncertainty = compute_uncertainty(velocity_samples, method=method)
