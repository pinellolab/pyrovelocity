...
class StandardDynamicsModel(BaseDynamicsModel):
    ...
    def _predict_future_states_impl(
        ...
        # Handle the case where beta == gamma
        s_future_beta_eq_gamma = s_ss + (s_current - s_ss) * exp_neg_gamma_t + \
                                (u_current - u_ss) * beta * time_delta * exp_neg_gamma_t
        
        s_future_beta_neq_gamma = s_ss + (s_current - s_ss) * exp_neg_gamma_t + \
                                 beta * (u_current - u_ss) / (gamma - beta_safe) * \
                                 (exp_neg_beta_t - exp_neg_gamma_t)
        
        s_future = jnp.where(beta_eq_gamma, s_future_beta_eq_gamma, s_future_beta_neq_gamma)
