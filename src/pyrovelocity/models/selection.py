...
class ModelSelection:
    ...
    def __init__(self, name: str = "model_selection"):
        ...
        """
        self.name = name
        self.comparison_tool = BayesianModelComparison(name=f"{name}_comparison")
    ...
    def select_model(
        ...
        if criterion == SelectionCriterion.WAIC:
            ...
            comparison_result = self.comparison_tool.compare_models_bayes_factors(
                models=models,
                data=data,
            )
        else:
...
class ModelEnsemble:
    ...
    def __post_init__(self):
        ...
        if set(self.weights.keys()) != set(self.models.keys()):
            raise ValueError("Model names in weights must match model names in models")
        ...
        **kwargs
        ...
        for model_name, model in self.models.items():
            ...
            if model_name not in posterior_samples:
                logger.warning(f"No posterior samples for model {model_name}, skipping")
                continue
            ...
            for i in range(len(next(iter(samples.values())))):
                ...
                if predictions is None:
                    raise ValueError(f"Model {model_name} did not return 'predictions' key")
        ...
        if len(all_predictions) > 1:
            model_means = np.array([np.mean(preds, axis=0) for preds in all_predictions])
    ...
    def from_selection_result(
        ...
        if use_weights:
            if metric_name in ["waic", "loo", "cv_error"]:
                ...
                epsilon = 1e-10
                raw_values = {name: 1.0 / (value + epsilon) 
            ...
            weights = {name: value / total for name, value in raw_values.items()}
        else:
            # Use equal weights
    ...
    def from_top_k_models(
        ...
        if metric_name in ["waic", "loo", "cv_error"]:
            ...
            sorted_models = sorted(comparison_result.values.items(), key=lambda x: x[1])
        else:
            # For other metrics, higher is better
            sorted_models = sorted(comparison_result.values.items(), key=lambda x: x[1], reverse=True)
        ...
        if use_weights:
            ...
            # Use equal weights
            weights = {name: 1.0 / len(filtered_models) for name in filtered_models}
    ...
    def _get_cv_splitter(
        ...
        if self.stratify_by is not None:
            ...
            if self.stratify_by not in adata.obs:
                raise ValueError(f"Stratification column '{self.stratify_by}' not found in adata.obs")
