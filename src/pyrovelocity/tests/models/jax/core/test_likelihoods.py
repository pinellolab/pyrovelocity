...
def test_poisson_likelihood_interface():
    ...
    expected_u_rate = (
        jnp.exp(scaling_params["u_log_library"][:, jnp.newaxis]) * ut
    )
    expected_s_rate = (
        jnp.exp(scaling_params["s_log_library"][:, jnp.newaxis]) * st
    )
...
def test_negative_binomial_likelihood_default_scaling():
    ...
    # Check rate parameter (1/(dispersion * mean))
    expected_u_rate = 1.0 / (
        jnp.ones_like(u_dist.rate)
        * jnp.exp(scaling_params["u_log_library"][:, jnp.newaxis])
        * ut
    )
    expected_s_rate = 1.0 / (
        jnp.ones_like(s_dist.rate)
        * jnp.exp(scaling_params["s_log_library"][:, jnp.newaxis])
        * st
    )
    assert jnp.allclose(u_dist.rate, expected_u_rate)
    assert jnp.allclose(s_dist.rate, expected_s_rate)
