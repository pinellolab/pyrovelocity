...
def test_convert_array():
    ...
    # Check values
    np.testing.assert_allclose(torch_tensor.numpy(), np.array([[1.0, 2.0], [3.0, 4.0]]))
...
def test_convert_parameters():
    ...
    # Check values
    np.testing.assert_allclose(jax_params["weight"], np.array([[1.0, 2.0], [3.0, 4.0]]))
...
def test_convert_parameters_to_torch():
    ...
    assert all(isinstance(param, torch.Tensor) for param in torch_params.values())
    
    # Check keys
    assert set(torch_params.keys()) == set(jax_params.keys())
    
    # Check values
    np.testing.assert_allclose(torch_params["weight"].numpy(), np.array([[1.0, 2.0], [3.0, 4.0]]))
    np.testing.assert_allclose(torch_params["bias"].numpy(), np.array([0.1, 0.2]))
...
def test_convert_model_state():
    ...
    assert all(isinstance(param, jnp.ndarray) for param in jax_model_state.parameters.values())
    np.testing.assert_allclose(jax_model_state.parameters["weight"], np.array([[1.0, 2.0], [3.0, 4.0]]))
    np.testing.assert_allclose(jax_model_state.parameters["bias"], np.array([0.1, 0.2]))
    
    # Check dynamics output
    assert isinstance(jax_model_state.dynamics_output, tuple)
    assert len(jax_model_state.dynamics_output) == 2
    np.testing.assert_allclose(jax_model_state.dynamics_output[0], np.array([[5.0, 6.0], [7.0, 8.0]]))
    np.testing.assert_allclose(jax_model_state.dynamics_output[1], np.array([[9.0, 10.0], [11.0, 12.0]]))
    
    # Check observations
    assert isinstance(jax_model_state.observations, dict)
    assert all(isinstance(obs, jnp.ndarray) for obs in jax_model_state.observations.values())
    np.testing.assert_allclose(jax_model_state.observations["counts"], np.array([[13.0, 14.0], [15.0, 16.0]]))
...
def test_convert_model_state_to_torch():
    ...
    assert all(isinstance(param, torch.Tensor) for param in torch_model_state["parameters"].values())
    np.testing.assert_allclose(torch_model_state["parameters"]["weight"].numpy(), np.array([[1.0, 2.0], [3.0, 4.0]]))
    np.testing.assert_allclose(torch_model_state["parameters"]["bias"].numpy(), np.array([0.1, 0.2]))
    
    # Check dynamics output
    assert isinstance(torch_model_state["dynamics_output"], tuple)
    assert len(torch_model_state["dynamics_output"]) == 2
    np.testing.assert_allclose(torch_model_state["dynamics_output"][0].numpy(), np.array([[5.0, 6.0], [7.0, 8.0]]))
    np.testing.assert_allclose(torch_model_state["dynamics_output"][1].numpy(), np.array([[9.0, 10.0], [11.0, 12.0]]))
    
    # Check observations
    assert isinstance(torch_model_state["observations"], dict)
    assert all(isinstance(obs, torch.Tensor) for obs in torch_model_state["observations"].values())
    np.testing.assert_allclose(torch_model_state["observations"]["counts"].numpy(), np.array([[13.0, 14.0], [15.0, 16.0]]))
...
def test_roundtrip_conversion():
    ...
    assert all(isinstance(param, torch.Tensor) for param in roundtrip_torch_params.values())
    
    # Check keys
    assert set(roundtrip_torch_params.keys()) == set(original_torch_params.keys())
