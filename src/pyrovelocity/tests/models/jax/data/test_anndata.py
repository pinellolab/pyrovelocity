...
def test_prepare_anndata(mock_anndata):
    ...
    assert (
        data_dict["X_unspliced"].shape == mock_anndata.layers["unspliced"].shape
    )
...
def test_store_results(mock_anndata):
    ...
    np.testing.assert_allclose(
        adata_out.layers["velocity_model_velocity"],
        np.array(results["velocity"]),
    )
    np.testing.assert_allclose(
        adata_out.var["velocity_model_alpha"], np.array(results["alpha"])
    )
    np.testing.assert_allclose(
        adata_out.var["velocity_model_beta"], np.array(results["beta"])
    )
    np.testing.assert_allclose(
        adata_out.var["velocity_model_gamma"], np.array(results["gamma"])
    )
    np.testing.assert_allclose(
        adata_out.obs["velocity_model_switching"],
        np.array(results["switching"]),
    )
    np.testing.assert_allclose(
        adata_out.obs["velocity_model_latent_time"],
        np.array(results["latent_time"]),
    )
...
def test_get_library_size(mock_anndata):
    ...
    # Check that the arrays contain the correct data
    expected_u_lib_size = jnp.array(
        mock_anndata.layers["unspliced"].sum(axis=1)
    ).flatten()
    expected_s_lib_size = jnp.array(
        mock_anndata.layers["spliced"].sum(axis=1)
    ).flatten()
...
def test_prepare_anndata_with_custom_layers(mock_anndata):
    ...
    # Rename the layers
    mock_anndata.layers["custom_spliced"] = mock_anndata.layers[
        "spliced"
    ].copy()
    mock_anndata.layers["custom_unspliced"] = mock_anndata.layers[
        "unspliced"
    ].copy()
    ...
    # Check that the arrays have the correct shape
    assert (
        data_dict["X_spliced"].shape
        == mock_anndata.layers["custom_spliced"].shape
    )
    assert (
        data_dict["X_unspliced"].shape
        == mock_anndata.layers["custom_unspliced"].shape
    )

    # Check that the arrays contain the correct data
    np.testing.assert_allclose(
        data_dict["X_unspliced"],
        jnp.array(mock_anndata.layers["custom_unspliced"]),
    )
    np.testing.assert_allclose(
        data_dict["X_spliced"], jnp.array(mock_anndata.layers["custom_spliced"])
    )
