...
def test_cross_validator_init(multiple_models):
    ...
    cv = CrossValidator(
        models=models_dict, n_splits=3, test_size=0.3, random_state=123
    )
...
def test_create_train_test_splits(multiple_models, mock_adata):
    ...
    for split in splits:
        ...
        assert all(
            0 <= idx < len(mock_adata.obs) for idx in split["train_indices"]
        )
        assert all(
            0 <= idx < len(mock_adata.obs) for idx in split["test_indices"]
        )

        # Verify no overlap between train and test indices
        assert set(split["train_indices"]).isdisjoint(
            set(split["test_indices"])
        )

        # Verify all indices are covered (train + test = total)
        assert len(split["train_indices"]) + len(split["test_indices"]) == len(
            mock_adata.obs
        )
...
def test_cross_validate(
    multiple_models, mock_adata, mock_cross_validation_result
):
    ...
    with patch.object(cv, "_create_train_test_splits") as mock_create_splits:
        ...
        with patch.object(cv, "_evaluate_model_fold") as mock_evaluate:
            ...
            assert mock_evaluate.call_count == len(models_dict) * len(
                mock_splits
            )
...
def test_select_best_model(
    ...
    with patch.object(
        cv, "cross_validate", return_value=mock_cross_validation_result
    ):
        # Select the best model based on metric_1
        best_model, avg_scores = cv.select_best_model(
            mock_adata, metric="metric_1"
        )
...
def test_compute_average_scores(multiple_models, mock_cross_validation_result):
    ...
    avg_scores = cv._compute_average_scores(
        mock_cross_validation_result, "metric_1"
    )
    ...
    with pytest.raises(KeyError):
        cv._compute_average_scores(
            mock_cross_validation_result, "nonexistent_metric"
        )
