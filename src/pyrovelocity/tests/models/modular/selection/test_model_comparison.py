...
def test_compute_waic(multiple_models):
    ...
    with patch("arviz.waic", return_value=mock_waic_result), patch.object(
        BayesianModelComparison,
        "_extract_log_likelihood",
        return_value=torch.ones(10, 10),
    ):
...
def test_compute_loo(multiple_models):
    ...
    with patch("arviz.loo", return_value=mock_loo_result), patch.object(
        BayesianModelComparison,
        "_extract_log_likelihood",
        return_value=torch.ones(10, 10),
    ):
...
def test_extract_log_likelihood(multiple_models):
    ...
    log_likelihood = comparison._extract_log_likelihood(
        model, posterior_samples=posterior_samples, data=data
    )
...
def test_compute_bayes_factor(multiple_models):
    ...
    with patch.object(
        comparison, "_compute_log_marginal_likelihood"
    ) as mock_compute:
        ...
        bf = comparison.compute_bayes_factor(
            model1=model1, model2=model2, data=data
        )
...
def test_compare_models(multiple_models):
    ...
    with patch("arviz.waic", side_effect=mock_waic_results), patch(
        "arviz.data.convert_to_dataset", return_value=MagicMock()
    ), patch.object(
        BayesianModelComparison,
        "_extract_log_likelihood",
        return_value=torch.ones(10, 10),
