...
def test_model_ensemble_init(multiple_models):
    ...
    expected_weights = {model.name: 1/len(models_dict) for model in multiple_models}
    assert ensemble.weights == expected_weights
    
    # Initialize with custom weights
    custom_weights = {model.name: weight for model, weight in zip(multiple_models, [0.5, 0.3, 0.2])}
...
def test_model_ensemble_with_invalid_models():
    ...
    with pytest.raises(TypeError, match="All models must be instances of PyroVelocityModel"):
...
def test_model_ensemble_with_invalid_weights(multiple_models):
    ...
    with pytest.raises(ValueError, match="Model names in weights must match model names in models"):
        incomplete_weights = {model_names[0]: 0.5, model_names[1]: 0.5}  # Missing model_names[2]
    ...
    with pytest.raises(ValueError, match="All weights must be non-negative"):
        negative_weights = {name: -0.1 if i == 0 else (0.55 if i == 1 else 0.55) for i, name in enumerate(model_names)}
...
def test_predict_method(multiple_models, sample_data):
    ...
    for i, (name, model) in enumerate(models_dict.items()):
        ...
        pred = torch.ones((sample_data["n_cells"], sample_data["n_genes"])) * (i + 1)
    ...
    expected_prediction = sum(weights_dict[name] * p for name, p in mock_predictions.items())
...
def test_predict_future_states_method(multiple_models, sample_data):
    ...
    # Verify the prediction is a weighted average of individual predictions
    expected_u_future = sum(weights_dict[name] * fs[0] for name, fs in mock_future_states.items())
    expected_s_future = sum(weights_dict[name] * fs[1] for name, fs in mock_future_states.items())
    ...
    for model in models_dict.values():
        model.predict_future_states.assert_called_once_with(current_state, time_delta)
...
def test_get_posterior_samples(multiple_models):
    ...
    for param in ["alpha", "beta", "gamma"]:
        ...
        assert samples[param].shape[0] == list(models_dict.values())[0].result.posterior_samples[param].shape[0]
        # Second dimension should be the sum of all model dimensions
        total_dim = sum(model.result.posterior_samples[param].shape[1] for model in models_dict.values())
        assert samples[param].shape[1] == total_dim


def test_calculate_weights_from_comparison(multiple_models, mock_comparison_result):
    ...
    assert weights[model_names[0]] > weights[model_names[1]] > weights[model_names[2]]
...
def test_normalize_weights():
    ...
    assert normalized == pytest.approx({"model1": 0.2, "model2": 0.3, "model3": 0.5})
    
    # Test with all zeros (should return equal weights)
    weights_dict = {"model1": 0, "model2": 0, "model3": 0}
    normalized = ModelEnsemble._normalize_weights(weights_dict)
    assert sum(normalized.values()) == pytest.approx(1.0)
    assert normalized == pytest.approx({"model1": 1/3, "model2": 1/3, "model3": 1/3})
