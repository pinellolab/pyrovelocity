...
def test_select_best_model(multiple_models, mock_comparison_result):
    ...
    with patch.object(
        ...
        assert result.selected_model_name == "model_0"  # First model has lowest WAIC
    ...
    with patch.object(
        ...
        assert result.selected_model_name == "model_0"  # First model has lowest LOO
...
def test_create_ensemble(multiple_models, mock_comparison_result, mock_selection_result):
...
def test_compare_models(multiple_models):
    ...
    posterior_samples = {f"model_{i}": {"param": torch.ones(5)} for i in range(3)}
    
    # Convert models list to dictionary
    models_dict = {model.name: model for model in multiple_models}
    
    # Mock the BayesianModelComparison.compute_waic method
    with patch("pyrovelocity.models.modular.comparison.BayesianModelComparison.compute_waic") as mock_compute:
