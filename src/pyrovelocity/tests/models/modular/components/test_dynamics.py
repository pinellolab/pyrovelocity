...
class TestStandardDynamicsModel:
    ...
    def test_conservation_laws(self, model, simple_params):
        ...
        # du/dt = alpha - beta * u
        expected_du_dt = (
            simple_params["alpha"] - simple_params["beta"] * u_t[:-1]
        )
        # ds/dt = beta * u - gamma * s
        expected_ds_dt = (
            simple_params["beta"] * u_t[:-1] - simple_params["gamma"] * s_t[:-1]
        )
...
class TestNonlinearDynamicsModel:
    ...
    def test_steady_state(self, model, simple_params):
        ...
        assert jnp.allclose(
            ...
            jnp.ones_like(u_ss_standard),
            rtol=1e-1,
        )
        assert jnp.allclose(
            s_ss_large_k / s_ss_standard,
            jnp.ones_like(s_ss_standard),
            rtol=1e-1,
        )
    ...
    def test_saturation_effects(self, model, simple_params):
        ...
        large_k_params["k_alpha"] = jnp.array(
            [1e6, 1e6]
        )  # Use very large values
        large_k_params["k_beta"] = jnp.array(
            [1e6, 1e6]
        )  # Use very large values
