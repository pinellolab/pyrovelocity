...
def test_likelihood_model_with_gene_offset(test_adata):
    ...
    piecewise_model = LikelihoodModelRegistry.get("piecewise_activation_poisson")()
    ...
    # Generate distributions
    piecewise_dists = piecewise_model(
    ...
    piecewise_rate = piecewise_dists["obs_counts"].rate
    legacy_rate = legacy_dists["obs_counts"].rate

    # Generate distributions without offset for comparison
    piecewise_dists_no_offset = piecewise_model(test_adata, cell_state=cell_state, batch_size=batch_size)
    legacy_dists_no_offset = legacy_model(test_adata, cell_state=cell_state, batch_size=batch_size)

    piecewise_rate_no_offset = piecewise_dists_no_offset["obs_counts"].rate
    ...
    assert torch.allclose(piecewise_rate, piecewise_rate_no_offset * 2.0, rtol=1e-5)
...
def test_likelihood_registry():
    ...
    assert "piecewise_activation_poisson" in LikelihoodModelRegistry.list_available()
