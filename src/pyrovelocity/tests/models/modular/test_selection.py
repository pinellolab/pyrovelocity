...
def test_model_selection_bayes_factor(
    mock_compare_models_bf,
    mock_models,
    mock_posterior_samples_dict,
    sample_data,
):
...
def test_model_ensemble_predict(
    mock_obs_forward,
    mock_dyn_forward,
    mock_models,
    mock_posterior_samples_dict,
...
def test_model_ensemble_from_selection_result(
    mock_models, mock_selection_result
):
...
def test_model_ensemble_from_top_k_models(mock_models, mock_selection_result):
    ...
    assert all(
        np.isclose(w, 0.5) for w in ensemble.weights.values()
    )  # Equal weights
    ...
    assert "model2" not in ensemble.models  # Worst model
    assert (
        ensemble.weights["model3"] > ensemble.weights["model1"]
    )  # model3 has better WAIC
