...
def test_likelihood_model_with_gene_offset(test_adata):
    ...
    latent_dim = 5
    cell_state = jnp.zeros((batch_size, latent_dim))
    gene_offset = jnp.ones((batch_size, test_adata.n_vars)) * 2.0  # Double the offset
    
    # Generate distributions
    poisson_dists = poisson_model(test_adata, cell_state=cell_state, gene_offset=gene_offset)
    nb_dists = nb_model(test_adata, cell_state=cell_state, gene_offset=gene_offset)
