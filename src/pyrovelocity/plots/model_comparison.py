...
def plot_model_comparison(
    ...
    if show_differences and comparison_result.differences:
        ...
        for i, (model1, diffs) in enumerate(
            comparison_result.differences.items()
        ):
            for j, (model2, diff) in enumerate(diffs.items()):
                ...
                if abs(diff) > threshold:
                    ...
                    ax.annotate(
                        ...
                        arrowprops=dict(
                            arrowstyle="<->",
                            color="red" if abs(diff) > threshold else "gray",
                        ),
                        ha="center",
                        va="bottom",
                        fontsize=8,
    ...
    if highlight_best:
        if is_significant:
            ax.bar(
                ...
                0,
                color="#2ca02c",
                label=f"Best model (significant: diff > {threshold})",
            )
        else:
            ax.bar(
                0,
                0,
                color="#ff7f0e",
                label=f"Best model (not significant: diff <= {threshold})",
            )
...
def plot_posterior_predictive_check(
    ...
    # Plot predicted mean
    ax.plot(
        np.arange(len(pred_mean_sorted)),
        pred_mean_sorted,
        "o-",
        alpha=0.7,
    ...
    coverage = np.mean(
        (observed_data >= pred_lower) & (observed_data <= pred_upper)
    )
...
def plot_diagnostic_metrics(
    ...
    for i, model in enumerate(model_names):
        ...
        min_vals = [
            min(models[m].get(metric, float("inf")) for m in model_names)
            for metric in metrics
        ]
        max_vals = [
            max(models[m].get(metric, 0) for m in model_names)
            for metric in metrics
        ]

        # Avoid division by zero
        ranges = [
            max(0.001, max_val - min_val)
            for min_val, max_val in zip(min_vals, max_vals)
        ]
        norm_values = [
            (val - min_val) / range_val
            for val, min_val, range_val in zip(values[:-1], min_vals, ranges)
        ]
        norm_values += norm_values[:1]  # Close the loop

        ax.plot(
            angles, norm_values, linewidth=2, linestyle="solid", label=model
        )
