...
{t^*}_j \mid Z_j = k &= \text{stick\_breaking}(0, T_{\max,k}) \label{eq-branch-stick-breaking}
\end{align}

Each branch $k$ has its own process duration $T_{\max,k}$ and stick-breaking temporal coordinate system with fixed anchors at $0$ and $T_{\max,k}$.

**Gene-specific parameters (shared across branches):**
\begin{align}
\gamma^*_i &\sim \text{LogNormal}(\mu_{\gamma^*}, \sigma_{\gamma^*}^2) \label{eq-shared-gamma} \\
U_{0i} &\sim \text{LogNormal}(\mu_{U_0}, \sigma_{U_0}^2) \label{eq-shared-U0}
\end{align}

**Branch-gene-specific parameters:**
\begin{align}
{t^*_{on}}_{ik} &\sim \text{Normal}(\mu_{on,k}, \sigma_{on}^2) \quad \text{for } i \in \{1, \ldots, G\}, k \in \{1, \ldots, K\} \label{eq-branch-onset} \\
\delta^*_{ik} &\sim \text{LogNormal}(\mu_{\delta,k}, \sigma_{\delta}^2) \label{eq-branch-duration-gene} \\
R_{on,ik} &\sim \text{LogNormal}(\mu_{R,k}, \sigma_R^2) \label{eq-branch-fold-change}
\end{align}

**Cell-specific capture efficiency:**
\begin{align}
\lambda_j &\sim \text{LogNormal}(\mu_{\lambda}, \sigma_{\lambda}^2) \label{eq-branch-lambda}
\end{align}

**Branch-specific piecewise analytical solutions:**

For cell $j$ assigned to branch $k$ (i.e., $Z_j = k$), the dimensionless concentrations follow the same piecewise analytical solutions as the single-branch model, but using branch-specific parameters:
...
The analytical solutions for each phase remain identical to the single-branch case, but now use branch-gene-specific parameters ${t^*_{on}}_{ik}$, $\delta^*_{ik}$, and $R_{on,ik}$.

**Mixture likelihood:**
\begin{align}
\mathcal{L}(\text{data}) &= \prod_{j=1}^N \sum_{k=1}^K \pi_k \cdot \mathcal{L}_k(\text{data}_j \mid \text{branch } k) \label{eq-mixture-likelihood}
\end{align}

where $\mathcal{L}_k(\text{data}_j \mid \text{branch } k)$ is the likelihood for cell $j$ under branch $k$ using branch-specific parameters.

**Observation model (unchanged):**
\begin{align}
u_{ij} &\sim \text{Poisson}(\lambda_j \cdot U_{0i} \cdot u^*_{ij}) \label{eq-branch-u-obs} \\
s_{ij} &\sim \text{Poisson}(\lambda_j \cdot U_{0i} \cdot s^*_{ij}) \label{eq-branch-s-obs}
\end{align}

where $u^*_{ij}$ and $s^*_{ij}$ are computed using the branch-specific piecewise analytical solutions for the branch to which cell $j$ is assigned.
...
For each branch $k \in \{1, \ldots, K\}$, cells assigned to that branch ($\{j : Z_j = k\}$) follow an independent stick-breaking process:

\begin{align}
\text{Let } \mathcal{N}_k &= |\{j : Z_j = k\}| \quad \text{(number of cells in branch } k\text{)} \\
\text{For cells } j_1, j_2, \ldots, j_{\mathcal{N}_k} &\text{ where } Z_{j_\ell} = k: \\
V_{k,\ell} &\sim \text{Beta}(1, \mathcal{N}_k - \ell) \quad \text{for } \ell \in \{1, \ldots, \mathcal{N}_k - 1\} \label{eq-branch-beta-variables} \\
\tilde{t}_{k,1} &= 0 \quad \text{(first cell at branch start)} \\
\tilde{t}_{k,\ell} &= \tilde{t}_{k,\ell-1} + V_{k,\ell-1} \prod_{m=1}^{\ell-2} (1 - V_{k,m}) \quad \text{for } \ell \in \{2, \ldots, \mathcal{N}_k\} \label{eq-branch-stick-construction} \\
{t^*}_{j_\ell} &= T_{\max,k} \times \tilde{t}_{k,\ell} \quad \text{(branch-specific scaling)} \label{eq-branch-time-scaling}
\end{align}
...
\begin{align}
0 = {t^*}_{j_1} < {t^*}_{j_2} < \cdots < {t^*}_{j_{\mathcal{N}_k}} = T_{\max,k}
\end{align}
...
- Fixed anchors: ${t^*}_{j_1} = 0$ and ${t^*}_{j_{\mathcal{N}_k}} = T_{\max,k}$
- No scaling symmetry within branch due to fixed endpoints
- Gene parameters ${t^*_{on}}_{ik}$, $\delta^*_{ik}$ are independent of $T_{\max,k}$
...
During MCMC sampling, branch assignments $Z_j$ are updated using standard categorical sampling:
\begin{align}
P(Z_j = k \mid \text{rest}) \propto \pi_k \cdot \mathcal{L}_k(\text{data}_j \mid \text{branch } k)
\end{align}
...
1. **Branch-parallel likelihood evaluation**:
   ```python
   # Compute likelihood for each branch in parallel
   branch_likelihoods = parallel_map(
       lambda k: compute_branch_likelihood(data, branch_k_params),
       branches
   )
   ```

2. **Gene-parallel parameter updates**:
   ```python
   # Update gene parameters across branches in parallel
   for k in branches:
       gene_params[k] = parallel_map(
           lambda i: update_gene_params(gene_i, branch_k),
           genes
       )
   ```

3. **Cell-parallel branch assignment updates**:
   ```python
   # Update branch assignments for cells in parallel
   new_assignments = parallel_map(
       lambda j: sample_branch_assignment(cell_j, branch_likelihoods),
       cells
   )
   ```
...
**Beta variable sampling:**
```python
# Ensure numerical stability in stick-breaking
def sample_beta_variables(branch_size):
    # Avoid extreme values that cause numerical issues
    alpha, beta = 1.0, max(branch_size - 1, 1.0)
    return np.clip(np.random.beta(alpha, beta), 1e-8, 1-1e-8)
```

**Likelihood computation:**
```python
# Use log-space arithmetic to prevent overflow/underflow
def compute_mixture_likelihood(data, branch_params, branch_probs):
    log_likelihoods = []
    for k, (params, prob) in enumerate(zip(branch_params, branch_probs)):
        log_lik = compute_log_likelihood(data, params) + np.log(prob)
...
- **Sparse representations**: Use sparse matrices for gene expression data
- **Branch indexing**: Maintain efficient mappings from cells to branches
- **Parameter caching**: Cache frequently accessed parameter combinations

**Memory-efficient sampling:**
```python
# Process branches sequentially to limit memory usage
def memory_efficient_sampling(data, max_memory_per_branch):
    for branch_k in branches:
        # Load only data for current branch
        branch_data = data[branch_assignments == k]
        # Process branch with limited memory footprint
        updated_params = sample_branch_parameters(branch_data)
        # Store results and free memory
        store_and_cleanup(updated_params)
```
...
def benchmark_branching_model(n_cells, n_genes, n_branches):
    start_time = time.time()

    # Generate synthetic data
    data = generate_branching_data(n_cells, n_genes, n_branches)

    # Fit model
    model = BranchingPyroVelocityModel()
    results = model.fit(data, num_samples=1000)
    ...
    return {
        ...
        'time_per_sample': total_time / 1000,
        'scalability_metrics': compute_scalability_metrics(results)
    }
```
