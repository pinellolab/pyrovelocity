...
- $0 < t^*_{on,i} < 0.3 \mathbb{E}[T_M]$ (early activation within process duration)
- $\delta^*_i < 0.4 \mathbb{E}[T_M]$ (short activation duration relative to process)
...
- $0 < t^*_{on,i} < 0.3 \mathbb{E}[T_M]$ (early activation within process duration)
- $\delta^*_i > 0.5 \mathbb{E}[T_M]$ (long activation duration relative to process)
...
**General principle:**
\begin{align}
t^*_{on,i} &\sim \text{Normal}(f_{on} \times \mathbb{E}[T_M], \sigma_{on}^2) \\
\delta^*_i &\sim \text{LogNormal}(\log(f_{\delta} \times \mathbb{E}[T_M]), \sigma_{\delta}^2)
\end{align}
...
- **Conservative (Gamma(5,1))**: $\mathbb{E}[T_M] = 5$ → $t^*_{on,i} \sim \text{Normal}(1.5, \sigma_{on}^2)$
- **Skeptical (Gamma(1,1))**: $\mathbb{E}[T_M] = 1$ → $t^*_{on,i} \sim \text{Normal}(0.3, \sigma_{on}^2)$
- **Permissive (Gamma(1,0.2))**: $\mathbb{E}[T_M] = 5$ → $t^*_{on,i} \sim \text{Normal}(1.5, \sigma_{on}^2)$
...
- **Transient**: $0 < t^*_{on,i} < 0.3 \mathbb{E}[T_M]$, $\delta^*_i < 0.4 \mathbb{E}[T_M]$, and $R_{\text{on},i} > 2.0$
- **Sustained**: $0 < t^*_{on,i} < 0.3 \mathbb{E}[T_M]$, $\delta^*_i > 0.5 \mathbb{E}[T_M]$, and $R_{\text{on},i} > 2.0$

where $\mathbb{E}[T_M] = \alpha_T/\beta_T$ for the Gamma prior, so these thresholds adapt to the chosen hyperparameters.
...
- **$t^*_{on,i} \sim \text{Normal}(\mu_{on}, \sigma_{on}^2)$**: Absolute activation onset time, typically centered around $0.3 \times \mathbb{E}[T_M]$ to expect activation at 30% of process duration. **Negative values** enable pre-activation scenarios where genes were already activated before observation began.

- **$\delta^*_i \sim \text{LogNormal}(\mu_{\delta}, \sigma_{\delta}^2)$**: Absolute activation duration, typically centered around $0.4 \times \mathbb{E}[T_M]$ to expect 40% duration activation. This enables both transient (short duration) and sustained (long duration) activation patterns.

**Pattern classification implications:**

- **Pre-activation** ($t^*_{on,i} < 0$): Probability depends on $\mu_{on}$ and $\sigma_{on}$ choices
- **Transient** ($0 < t^*_{on,i} < 0.3 \mathbb{E}[T_M]$ and $\delta^*_i < 0.4 \mathbb{E}[T_M]$): Balanced probability for observable complete cycles
- **Sustained** ($0 < t^*_{on,i} < 0.3 \mathbb{E}[T_M]$ and $\delta^*_i > 0.5 \mathbb{E}[T_M]$): Balanced probability for long-duration activation
