...
**Activation Onset Times**: $t^*_{on,i} \sim \text{Normal}(\mu_{on}, \sigma_{on}^2)$

- **Dimensionless interpretation**: Gene $i$ activates at $t^*_{on,i}$ characteristic timescales after process initiation
- **Real-time interpretation**: Gene $i$ activates at $t^*_{on,i}/\alpha_{\text{ref}}$ hours after process start
- **Relative interpretation**: Gene $i$ activates at $(t^*_{on,i}/T_M) \times 100\%$ through the process

**Example interpretations:**

- $t^*_{on,i} = 1.5$, $T_M = 5.0$: Gene activates 30% through the process
- $t^*_{on,i} = -0.5$: Gene was already active 0.5 timescales before observation began
...
- Gene with $t^*_{on,i} = 2.0$: Activates at $2.0 \times 8 = 16$ hours
...
- $t^*_{on,i}$: When a gene's regulatory program activates (intrinsic timing)
...
\begin{align}
t^*_{on,i} &\sim \text{Normal}(1.5, 0.8^2) \quad \text{(fixed biological timing)} \\
