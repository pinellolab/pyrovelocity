...
- **Parameters**: $\gamma^*_i$, $U_{0i}$, $\lambda_j$, plus hierarchical time structure ($T_{max}^*$, $t_{loc}$, $t_{scale}$, $\tilde{t}_j$)
...
# hyperparameters for hierarchical time structure (moved further left)
pgm.add_node("mu_T", r"$\mu_{T}$", 5.5, 6, fixed=True)
pgm.add_node("sigma_T", r"$\sigma_{T}^2$", 6, 6, fixed=True)
pgm.add_node("mu_t_loc", r"$\mu_{t_{loc}}$", 6.5, 6, fixed=True)
pgm.add_node("sigma_t_loc", r"$\sigma_{t_{loc}}^2$", 7, 6, fixed=True)
pgm.add_node("mu_t_scale", r"$\mu_{t_{scl}}$", 7.5, 6, fixed=True)
pgm.add_node("sigma_t_scale", r"$\sigma_{t_{scl}}^2$", 8, 6, fixed=True)
...
pgm.add_edge("mu_T", "T_max_star", plot_params=edge_params)
pgm.add_edge("sigma_T", "T_max_star", plot_params=edge_params)
pgm.add_edge("mu_t_loc", "t_loc", plot_params=edge_params)
pgm.add_edge("sigma_t_loc", "t_loc", plot_params=edge_params)
pgm.add_edge("mu_t_scale", "t_scale", plot_params=edge_params)
pgm.add_edge("sigma_t_scale", "t_scale", plot_params=edge_params)
...
\begin{align}
    T_{max}^* &\sim \text{Gamma}(\alpha_{T}, \beta_{T}) \label{eq-T-max-prior} \\
    t_{loc} &\sim \text{Gamma}(\alpha_{t_{loc}}, \beta_{t_{loc}}) \label{eq-t-loc-prior} \\
    t_{scale} &\sim \text{Gamma}(\alpha_{t_{scale}}, \beta_{t_{scale}}) \label{eq-t-scale-prior} \\
    \tilde{t}_j &\sim \text{Normal}(t_{loc}, t_{scale}^2) \quad \text{for } j \in \{1, \ldots, N\} \label{eq-t-tilde-prior} \\
    t^*_j &= T_{max}^* \times \tilde{t}_j \quad \text{(deterministic transformation)} \label{eq-t-star-transform}
\end{align}

$\tilde{t}_j$ represents the **relative temporal position** of cell $j$ (which can be negative for cells before the process start or greater than 1 for cells after the typical process duration), and $t^*_j$ represents the **actual dimensionless time coordinate** obtained by scaling the relative position by the global time scale $T_{max}^*$.
...
**$T_{max}^*$ (Global dimensionless time scale)**
...
- **Model role**: Multiplicative scaling factor in $t^*_j = T_{max}^* \times \tilde{t}_j$
- **Prior**: $T_{max}^* \sim \text{Gamma}(\alpha_{T}, \beta_{T})$ ensures positive values with flexible tail behavior
- **Example**: $T_{max}^* = 5$ means the process typically spans ~5 characteristic splicing times
...
**$t_{scale}$ (Population relative time scale)**
...
- **Prior**: $t_{scale} \sim \text{Gamma}(\alpha_{t_{scale}}, \beta_{t_{scale}})$ ensures positive scale with flexible shape
- **Statistical role**: Controls the amount of shrinkage toward the population mean
- **Example**: $t_{scale} = 0.2$ indicates relatively tight temporal clustering around $t_{loc}$

#### **Cell-Specific Time Coordinates**

**$\tilde{t}_j$ (Cell-specific relative time)**

- **Definition**: Latent relative time coordinate for cell $j$
- **Units**: Dimensionless (relative time units)
...
- **Prior**: $\tilde{t}_j \sim \text{Normal}(t_{loc}, t_{scale}^2)$ allows negative values (before process) and values >1 (after process)
- **Flexibility**: Can represent cells outside the typical process timeline
- **Example**: $\tilde{t}_j = -0.2$ means this cell is before the typical process start; $\tilde{t}_j = 1.3$ means after typical completion

**$t^*_j$ (Cell-specific dimensionless time)**

- **Definition**: Actual dimensionless time coordinate for cell $j$
- **Units**: Dimensionless (in units of characteristic splicing time)
...
- **Transformation**: $t^*_j = T_{max}^* \times \tilde{t}_j$ (deterministic scaling)
- **Example**: If $T_{max}^* = 5$ and $\tilde{t}_j = 0.6$, then $t^*_j = 3.0$ characteristic splicing times
...
- **Clear parameter roles**: $T_{max}^*$ controls global scale, $t_{loc}$ and $t_{scale}$ control population distribution
...
- **Flexible temporal range**: Cells can be before ($\tilde{t}_j < 0$) or after ($\tilde{t}_j > 1$) typical process timeline
- **Natural biological variation**: No artificial hard boundaries on temporal positions
...
- **Multiplicative structure**: Simple deterministic transformation $t^*_j = T_{max}^* \times \tilde{t}_j$
