...
- **Time coordinates**: Independent latent $t^*_j$ coordinates sampled uniformly within process duration $T_M$
- **Dynamics**: Piecewise constant transcription with three phases (off, on, return-to-off)
- **Parameters**: $\gamma^*_i$, $R_{\text{on},i}$ (fold-change), $t^*_{on,i}$ (onset), $\delta^*_i$ (duration), with independent temporal structure
...
1.0 & \text{if } t^* < t^*_{on,i}, \\
R_{\text{on},i} & \text{if } t^*_{on,i} \leq t^* < t^*_{on,i} + \delta^*_i, \\
1.0 & \text{if } t^* \geq t^*_{on,i} + \delta^*_i
\end{cases}$$
...
- $t^*_{on,i}$ is the dimensionless activation onset time (can be negative for pre-activation scenarios)
...
**Note**: This phase may not be observed if $t^*_{on,i} < 0$ (pre-activation scenario).

#### **Phase 2: On State** ($t^*_{on,i} \leq t^* < t^*_{on,i} + \delta^*_i$)

Let $\tau_{\text{on}} = t^* - t^*_{on,i}$ be the time since activation onset. The system transitions from the off-state steady state toward the on-state steady state.
...
#### **Phase 3: Return to Off State** ($t^* \geq t^*_{on,i} + \delta^*_i$)

Let $\tau_{\text{off}} = t^* - (t^*_{on,i} + \delta^*_i)$ be the time since returning to the off state. The system transitions from the end-of-phase-2 state back toward the off-state steady state.
...
With independent parameterization, gene timing parameters should be calibrated to the expected process duration based on chosen $T_M$ hyperparameters:

**General principle:**
\begin{align}
t^*_{on,i} &\sim \text{Normal}(f_{on} \times \text{E}[T_{\max}], \sigma_{on}^2) \\
\delta^*_i &\sim \text{LogNormal}(\log(f_{\delta} \times \text{E}[T_{\max}]), \sigma_{\delta}^2)
\end{align}

where $f_{on} = 0.3$ (expect activation at 30% of process) and $f_{\delta} = 0.4$ (expect 40% duration activation) are scaling factors based on biological expectations.

**Concrete examples for different experimental contexts:**
- **Conservative (Gamma(5,1))**: E[$T_{\max}$] = 5 → $t^*_{on,i} \sim \text{Normal}(1.5, \sigma_{on}^2)$
- **Skeptical (Gamma(1,1))**: E[$T_{\max}$] = 1 → $t^*_{on,i} \sim \text{Normal}(0.3, \sigma_{on}^2)$
- **Permissive (Gamma(1,0.2))**: E[$T_{\max}$] = 5 → $t^*_{on,i} \sim \text{Normal}(1.5, \sigma_{on}^2)$
...
# Adaptive to chosen T_M hyperparameters
alpha_T, beta_T = 5.0, 1.0  # Conservative prior example
E_T_M = alpha_T / beta_T  # Expected process duration
priors = {
    'R_on': {'loc': 0.916, 'scale': 0.4},                           # log(2.5), fold-change (LogNormal)
    't_on_star': {'loc': 0.3 * E_T_M, 'scale': 0.8},               # Normal(0.3*E[T_M], 0.8²), absolute onset
    'delta_star': {'loc': np.log(0.4 * E_T_M), 'scale': 0.45},     # log(0.4*E[T_M]), absolute duration (LogNormal)
}
...
patterns = {
    ...
    'transient': [
        ...
        ('t_on_star', '<', 0.3 * E_T_M),           # Early enough to complete cycle
        ('delta_star', '<', 0.4 * E_T_M),          # Short pulse duration
        ('R_on', '>', 2.0),                        # Sufficient fold change to observe
    ],
    'sustained': [
        ('t_on_star', '>', 0.0),                    # Activation within observation window
        ('t_on_star', '<', 0.3 * E_T_M),           # Early activation onset
        ('delta_star', '>', 0.5 * E_T_M),          # Long activation duration
...
### Dimensionless Framework and Biological Timescale Connection

The dimensionless parameterization requires careful interpretation to connect mathematical parameters to biological reality. This section clarifies the relationship between dimensionless temporal parameters and real biological timescales.

#### **Fundamental Dimensionless Transformation**

The dimensionless framework transforms real time into natural units based on characteristic biological rates:

$$t^* = t_{\text{real}} \times \alpha_{\text{ref}}$$

where:
- $t_{\text{real}}$ is time in physical units (hours, days, etc.)
- $\alpha_{\text{ref}} = \alpha^*_{\text{off}} = 1.0$ is the reference transcription rate (fixed)
- $t^*$ is dimensionless time in units of characteristic process rates

**Key insight**: All temporal parameters are expressed as multiples of the natural timescale $1/\alpha_{\text{ref}}$, where the characteristic timescale is typically the splicing/degradation timescale.

#### **Process Duration Parameter: Biological Interpretation**

The process duration $T_{\max}$ represents how many characteristic timescales the biological process spans:

$$T_{\max} = \frac{\text{Biological process duration}}{\text{Characteristic timescale of cellular rates}}$$

**Biological interpretation examples:**
- $T_{\max} = 1$: Process duration equals one characteristic timescale
- $T_{\max} = 5$: Process duration equals five characteristic timescales
- $T_{\max} = 0.5$: Process duration is half a characteristic timescale

**Steady-state achievement**: The fraction of steady-state dynamics observable is $1 - e^{-T_{\max}}$:
- $T_{\max} = 1$: Observes 63% approach to steady state
- $T_{\max} = 3$: Observes 95% approach to steady state
- $T_{\max} = 5$: Observes >99% approach to steady state

#### **Connection to Real Timescales During Model Fitting**

When fitting the model to experimental data, the inferred $T_{\max}$ reveals the underlying biological rates:

$$\text{Inferred characteristic timescale} = \frac{\text{Known experimental duration}}{T_{\max}^{\text{posterior}}}$$

**Practical example:**
- **Experimental setup**: 24-hour differentiation time course
- **Posterior estimate**: $T_{\max} \approx 3.0$
- **Inferred characteristic timescale**: $24/3 = 8$ hours
- **Biological interpretation**: Cellular processes (splicing, degradation) occur on ~8 hour timescales

This bidirectional relationship enables both forward simulation (given biological rates) and inverse inference (estimating rates from data).

#### **Gene Parameter Interpretation in Dimensionless Framework**

**Activation Onset Times**: $t^*_{0,\text{on}i} \sim \text{Normal}(\mu_{on}, \sigma_{on}^2)$

- **Dimensionless interpretation**: Gene $i$ activates at $t^*_{0,\text{on}i}$ characteristic timescales after process initiation
- **Real-time interpretation**: Gene $i$ activates at $t^*_{0,\text{on}i}/\alpha_{\text{ref}}$ hours after process start
- **Relative interpretation**: Gene $i$ activates at $(t^*_{0,\text{on}i}/T_{\max}) \times 100\%$ through the process

**Example interpretations:**
- $t^*_{0,\text{on}i} = 1.5$, $T_{\max} = 5.0$: Gene activates 30% through the process
- $t^*_{0,\text{on}i} = -0.5$: Gene was already active 0.5 timescales before observation began

**Activation Durations**: $\delta^*_i \sim \text{LogNormal}(\mu_{\delta}, \sigma_{\delta}^2)$

- **Dimensionless interpretation**: Gene $i$ remains active for $\delta^*_i$ characteristic timescales
- **Real-time interpretation**: Gene $i$ remains active for $\delta^*_i/\alpha_{\text{ref}}$ hours
- **Relative interpretation**: Gene $i$ remains active for $(\delta^*_i/T_{\max}) \times 100\%$ of the process

#### **Hyperparameter Choice as Biological Prior Knowledge**

The choice of $T_{\max}$ hyperparameters encodes specific biological assumptions about process duration:

**Conservative Prior: $T_{\max} \sim \text{Gamma}(5.0, 1.0)$**

- $\mathbb{E}[T_{\max}] = 5$, $\text{Var}[T_{\max}] = 5$
- **Biological assumption**: "I expect this process to span approximately 5 characteristic timescales, allowing observation of near-complete dynamics"
- **Implications**: If experimental duration is 20 hours, assumes characteristic timescale ≈ 4 hours
- **Expectation**: Observes 99% approach to steady state ($1 - e^{-5} \approx 0.99$)
- **Data burden**: Data must provide strong evidence if process is much shorter or longer

**Permissive Prior: $T_{\max} \sim \text{Gamma}(1.0, 0.2)$**

- $\mathbb{E}[T_{\max}] = 5$, $\text{Var}[T_{\max}] = 25$
- **Biological assumption**: "I'm uncertain about process duration - it could range from rapid (1 timescale) to very slow (15+ timescales)"
- **Implications**: Wide uncertainty allows data to determine appropriate timescale
- **Flexibility**: Can accommodate both early transient dynamics and complete processes
- **Data burden**: Minimal prior bias on process duration

**Skeptical Prior: $T_{\max} \sim \text{Gamma}(1.0, 1.0)$**

- $\mathbb{E}[T_{\max}] = 1$, $\text{Var}[T_{\max}] = 1$
- **Biological assumption**: "I expect rapid cellular responses, likely observing only early dynamics"
- **Implications**: Assumes observation of primarily transient phase (63% approach: $1 - e^{-1} \approx 0.63$)
- **Data burden**: Data must strongly support longer timescales if present
- **Use cases**: Stress responses, drug treatments, acute perturbations
...
#### **Model Fitting and Timescale Inference**

The dimensionless framework creates a bidirectional relationship between process duration and characteristic rates:

**Forward (simulation):**

1. Choose biological timescales (e.g., β = splicing rate)
2. Set process duration in real time (e.g., 24 hours)
3. Compute $T_{\max} = t_{\text{process}} \times β$
4. Generate dimensionless dynamics

**Inverse (inference from data):**

1. Observe expression data over known duration
2. Infer $T_{\max}$ from temporal patterns
3. Estimate characteristic rates: $\hat{β} = T_{\max}/t_{\text{experiment}}$
4. Convert all dimensionless parameters to real time

**Practical Example:**

**Experimental setup:** 48-hour differentiation time course

**Model fitting results:**

- Posterior: $T_{\max} \approx 6.0 \pm 0.5$
- Inferred characteristic timescale: $48/6 = 8$ hours
- Gene with $t^*_{0,\text{on}i} = 2.0$: Activates at $2.0 \times 8 = 16$ hours
- Gene with $\delta^*_i = 1.5$: Active for $1.5 \times 8 = 12$ hours

#### **Independent Parameterization Justification**

In the dimensionless framework, **gene timing parameters represent intrinsic biological properties** that should not scale with the arbitrary choice of observation window:

- $t^*_{0,\text{on}i}$: When a gene's regulatory program activates (intrinsic timing)
- $\delta^*_i$: How long a gene's regulatory program lasts (intrinsic duration)

**These are biological constants** that don't change based on how long we choose to observe the system. Therefore:

\begin{align}
t^*_{0,\text{on}i} &\sim \text{Normal}(1.5, 0.8^2) \quad \text{(fixed biological timing)} \\
\delta^*_i &\sim \text{LogNormal}(0.0, 0.45^2) \quad \text{(fixed biological duration)}
\end{align}

**NOT dependent on $T_{\max}$:**
\begin{align}
t^*_{0,\text{on}i} &\not\sim \text{Normal}(0.3 \times T_{\max}, 0.8^2) \quad \text{(would create artificial scaling)}
\end{align}

This independence ensures that gene regulatory timing is treated as an intrinsic biological property rather than an artifact of experimental design.
...
1. **Gene parameters are sampled independently**: $t^*_{0,\text{on}i} \not\propto T_{\max}$ and $\delta^*_i \not\propto T_{\max}$

2. **Scaling cell times but not gene parameters changes piecewise phase assignments**:
   - Original: Cell $j$ in phase determined by comparing $t^*_j$ vs. $t^*_{0,\text{on}i}$
   - Scaled: Cell $j$ in phase determined by comparing $\lambda t^*_j$ vs. $t^*_{0,\text{on}i}$ (unchanged)

3. **The piecewise analytical solutions depend on comparisons like $t^*_j$ vs. $t^*_{0,\text{on}i}$**:
   - Phase boundaries change when cell times are scaled but gene times are not
   - Different phase assignments lead to different analytical solution forms

4. **Therefore**: $\mathcal{L}(T_{\max}, \{t^*_j\}, \{t^*_{0,\text{on}i}\}, \{\delta^*_i\}) \neq \mathcal{L}(\lambda T_{\max}, \{\lambda t^*_j\}, \{t^*_{0,\text{on}i}\}, \{\delta^*_i\})$ □
