...
from pyrovelocity.models.modular.data.synthetic import PiecewisePatternGenerator
from pyrovelocity.models.modular.validation import (
    PiecewiseActivationValidator,
    generate_validation_parameter_sets,
)
...
We generate synthetic datasets representing each of the four gene expression patterns with known ground truth parameters:

```python
# Initialize pattern generator
pattern_generator = PiecewisePatternGenerator(seed=42)

# Generate validation parameter sets (2 sets per pattern = 8 total)
validation_parameter_sets = generate_validation_parameter_sets()

# Generate synthetic datasets for each parameter set
synthetic_datasets = {}
true_parameters = {}

for param_set in validation_parameter_sets:
    pattern = param_set["pattern"]
    ...
    if pattern == "activation":
        adata = pattern_generator.generate_activation_pattern(
            num_genes=5,
            num_cells=200,
            parameters=param_set
        )
    elif pattern == "decay":
        adata = pattern_generator.generate_decay_pattern(
            num_genes=5,
            num_cells=200,
            parameters=param_set
        )
    elif pattern == "transient":
        adata = pattern_generator.generate_transient_pattern(
            num_genes=5,
            num_cells=200,
            parameters=param_set
        )
    elif pattern == "sustained":
        adata = pattern_generator.generate_sustained_pattern(
            num_genes=5,
            num_cells=200,
            parameters=param_set
        )
    ...
    synthetic_datasets[dataset_key] = adata
    true_parameters[dataset_key] = adata.uns["true_parameters"]

    print(f"  Generated {adata.n_obs} cells × {adata.n_vars} genes")
    print(f"  Pattern parameters: {param_set}")
...
prior_predictive_samples = model.generate_prior_predictive_samples(
    num_samples=1000,
    num_genes=10,
    num_cells=200
)
...
fig_prior = plot_prior_predictive_checks(
    model=model,
    prior_samples=prior_predictive_samples,
    figsize=(15, 10)
)
...
for param_name, samples in prior_predictive_samples.items():
...
for dataset_key, adata in synthetic_datasets.items():
    print(f"\n{'='*60}")
    ...
    result = validator.validate_parameter_recovery(
        ...
        adata=adata,
        true_parameters=true_parameters[dataset_key],
...
for pattern in ["activation", "decay", "transient", "sustained"]:
    ...
    pattern_datasets = {k: v for k, v in synthetic_datasets.items() if k.startswith(pattern)}
...
for gamma_val in gamma_values:
    ...
    # Generate test data with specific gamma value
    test_adata = pattern_generator.generate_activation_pattern(
        num_genes=3,
        num_cells=100,
        parameters={
            "alpha_off": 0.1,
...
for config in benchmark_configs:
    ...
    test_adata = pattern_generator.generate_activation_pattern(
        ...
        parameters=validation_parameter_sets[0]  # Use first parameter set
    )
