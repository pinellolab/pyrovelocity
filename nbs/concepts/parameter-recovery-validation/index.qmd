...
**General principle:**
\begin{align}
t^*_{on,i} &\sim \text{Normal}(f_{on} \times \text{E}[T_M], \sigma_{on}^2) \\
\delta^*_i &\sim \text{LogNormal}(\log(f_{\delta} \times \text{E}[T_M]), \sigma_{\delta}^2)
\end{align}
...
- **Conservative (Gamma(5,1))**: E[$T_M$] = 5 → $t^*_{on,i} \sim \text{Normal}(1.5, \sigma_{on}^2)$
- **Skeptical (Gamma(1,1))**: E[$T_M$] = 1 → $t^*_{on,i} \sim \text{Normal}(0.3, \sigma_{on}^2)$
- **Permissive (Gamma(1,0.2))**: E[$T_M$] = 5 → $t^*_{on,i} \sim \text{Normal}(1.5, \sigma_{on}^2)$
...
The process duration $T_M$ represents how many characteristic timescales the biological process spans:

$$T_M = \frac{\text{Biological process duration}}{\text{Characteristic timescale of cellular rates}}$$

**Biological interpretation examples:**

- $T_M = 1$: Process duration equals one characteristic timescale
- $T_M = 5$: Process duration equals five characteristic timescales
- $T_M = 0.5$: Process duration is half a characteristic timescale

**Steady-state achievement**: The fraction of steady-state dynamics observable is $1 - e^{-T_M}$:
- $T_M = 1$: Observes 63% approach to steady state
- $T_M = 3$: Observes 95% approach to steady state
- $T_M = 5$: Observes >99% approach to steady state
...
When fitting the model to experimental data, the inferred $T_M$ reveals the underlying biological rates:

$$\text{Inferred characteristic timescale} = \frac{\text{Known experimental duration}}{T_M^{\text{posterior}}}$$

**Practical example:**
- **Experimental setup**: 24-hour differentiation time course
- **Posterior estimate**: $T_M \approx 3.0$
...
- **Relative interpretation**: Gene $i$ activates at $(t^*_{0,\text{on}i}/T_M) \times 100\%$ through the process

**Example interpretations:**
- $t^*_{0,\text{on}i} = 1.5$, $T_M = 5.0$: Gene activates 30% through the process
...
- **Relative interpretation**: Gene $i$ remains active for $(\delta^*_i/T_M) \times 100\%$ of the process

#### **Hyperparameter Choice as Biological Prior Knowledge**

The choice of $T_M$ hyperparameters encodes specific biological assumptions about process duration:

**Conservative Prior: $T_M \sim \text{Gamma}(5.0, 1.0)$**

- $\mathbb{E}[T_M] = 5$, $\text{Var}[T_M] = 5$
...
**Permissive Prior: $T_M \sim \text{Gamma}(1.0, 0.2)$**

- $\mathbb{E}[T_M] = 5$, $\text{Var}[T_M] = 25$
...
**Skeptical Prior: $T_M \sim \text{Gamma}(1.0, 1.0)$**

- $\mathbb{E}[T_M] = 1$, $\text{Var}[T_M] = 1$
...
3. Compute $T_M = t_{\text{process}} \times β$
4. Generate dimensionless dynamics

**Inverse (inference from data):**

1. Observe expression data over known duration
2. Infer $T_M$ from temporal patterns
3. Estimate characteristic rates: $\hat{β} = T_M/t_{\text{experiment}}$
...
- Posterior: $T_M \approx 6.0 \pm 0.5$
...
**NOT dependent on $T_M$:**
\begin{align}
t^*_{0,\text{on}i} &\not\sim \text{Normal}(0.3 \times T_M, 0.8^2) \quad \text{(would create artificial scaling)}
\end{align}
...
4. **Therefore**: $\mathcal{L}(T_M, \{t^*_j\}, \{t^*_{0,\text{on}i}\}, \{\delta^*_i\}) \neq \mathcal{L}(\lambda T_M, \{\lambda t^*_j\}, \{t^*_{0,\text{on}i}\}, \{\delta^*_i\})$ □
