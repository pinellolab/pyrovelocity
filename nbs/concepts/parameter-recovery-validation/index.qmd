...
\begin{align}
p(\tilde{y}) &= \int p(\tilde{y} | \gamma^*_i, R_{\text{on},i}, \tilde{t}^*_{0,\text{on}i}, \tilde{\delta}^*_i, U_{0i}, T_{M}^*, t_{loc}, t_{scl}, t^*_j, \lambda_j) \times \nonumber \\
&\quad p(\gamma^*_i, R_{\text{on},i}, \tilde{t}^*_{0,\text{on}i}, \tilde{\delta}^*_i, U_{0i}, T_{M}^*, t_{loc}, t_{scl}, t^*_j, \lambda_j) \, d\theta \label{eq-prior-predictive-piecewise}
\end{align}
...
\begin{align}
p(\tilde{y}) &= \int \int \int \int \int p(\tilde{y} | T_{M}^*, \theta_{\text{gene}}, \theta_{\text{cell}}, \theta_{\text{tech}}) \times \nonumber \\
&\quad p(T_{M}^*) \, p(\theta_{\text{gene}}) \, p(\theta_{\text{cell}}) \, p(\theta_{\text{tech}}) \, dT_{M}^* \, d\theta_{\text{gene}} \, d\theta_{\text{cell}} \, d\theta_{\text{tech}} \label{eq-hierarchical-integration}
\end{align}

where:

- $\theta_{\text{gene}} = \{\gamma^*_i, R_{\text{on},i}, \tilde{t}^*_{0,\text{on}i}, \tilde{\delta}^*_i, U_{0i}\}_{i=1}^G$ represents gene-specific parameters
- $\theta_{\text{cell}} = \{t_{loc}, t_{scl}, \tilde{t}_j, t^*_j\}_{j=1}^N$ represents cell-specific temporal parameters
- $\theta_{\text{tech}} = \{\lambda_j\}_{j=1}^N$ represents technical efficiency parameters

This decomposition reflects the model's assumption that gene-specific kinetics, population-level temporal structure, and technical factors contribute independently to the prior uncertainty.
...
1. **Sample hierarchical time parameters**:
   \begin{align}
   {T_{M}^*}^{(s)} &\sim \text{Gamma}(\alpha_{T}, \beta_{T}), \quad {t_{loc}}^{(s)} \sim \text{Gamma}(\alpha_{t_{loc}}, \beta_{t_{loc}}), \quad {t_{scl}}^{(s)} \sim \text{Gamma}(\alpha_{t_{scl}}, \beta_{t_{scl}})
   \end{align}

2. **Sample cell-specific relative times**:
   \begin{align}
   {\tilde{t}_j}^{(s)} &\sim \text{Normal}({t_{loc}}^{(s)}, ({t_{scl}}^{(s)})^2) \quad \text{for } j \in \{1, \ldots, N\}
   \end{align}

3. **Transform to absolute times**:
   \begin{align}
   {t^{*}_j}^{(s)} &= {T_{M}^*}^{(s)} \times \max({\tilde{t}_j}^{(s)}, \epsilon)
   \end{align}

4. **Sample gene-specific parameters**:
   \begin{align}
   {\gamma^{*}_i}^{(s)} &\sim \text{LogNormal}(\log(1.0), 0.5^2) \\
   {R_{\text{on},i}}^{(s)} &\sim \text{LogNormal}(\log(2.5), 0.4^2) \\
   {\tilde{t}^*_{0,\text{on}i}}^{(s)} &\sim \text{Normal}(0.5, 0.8^2) \\
   {\tilde{\delta}^*_i}^{(s)} &\sim \text{LogNormal}(\log(0.45), 0.45^2) \\
   {U_{0i}}^{(s)} &\sim \text{LogNormal}(\mu_{U_0}, \sigma_{U_0}^2)
   \end{align}

5. **Transform relative temporal parameters**:
   \begin{align}
   {t^*_{0,\text{on}i}}^{(s)} &= {T_{M}^*}^{(s)} \times {\tilde{t}^*_{0,\text{on}i}}^{(s)}, \quad {\delta^*_i}^{(s)} = {T_{M}^*}^{(s)} \times {\tilde{\delta}^*_i}^{(s)}
   \end{align}

6. **Sample capture efficiencies**:
   \begin{align}
   {\lambda_j}^{(s)} &\sim \text{LogNormal}(\mu_{\lambda}, \sigma_{\lambda}^2)
   \end{align}

7. **Compute latent concentrations**:
   \begin{align}
   {u^{*}_{ij}}^{(s)}, {s^{*}_{ij}}^{(s)} &= f_{\text{piecewise}}({t^*_j}^{(s)}, {\gamma^*_i}^{(s)}, {R_{\text{on},i}}^{(s)}, {t^*_{0,\text{on}i}}^{(s)}, {\delta^*_i}^{(s)})
   \end{align}

8. **Sample observations**:
   \begin{align}
...
1. **Global temporal scale uncertainty**: Uncertainty in $T_{M}^*$ affects the absolute timescale of all temporal processes, scaling both activation onset times and durations proportionally

2. **Population temporal distribution uncertainty**: Uncertainty in $t_{loc}, t_{scl}$ affects the distribution of cells across the relative temporal coordinate, determining population-level temporal heterogeneity
...
4. **Gene-specific temporal pattern uncertainty**: Prior uncertainty in $\tilde{t}^*_{0,\text{on}i}, \tilde{\delta}^*_i$ affects when activation occurs and how long it lasts relative to the population timeline
...
- **Temporal scaling interactions**: Uncertainty in $T_{M}^*$ interacts multiplicatively with uncertainty in relative temporal parameters, creating complex correlation structures
