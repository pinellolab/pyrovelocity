...
- **Key edge**: $T_{M}^* \rightarrow \Delta_i$ represents the scaling relationship for all temporal parameters
...
- **$\tilde{t}^*_{0,\text{on}i} \sim \text{Normal}(0.5, 0.8^2)$**: Centers relative activation onset at 50% through the process timeline, with 95% between approximately -1.1 and +2.1. **Negative values** enable pre-activation scenarios.

- **$\tilde{\delta}^*_i \sim \text{LogNormal}(\log(0.45), 0.45^2)$**: Centers relative activation duration at 45% of the process timeline, with 95% between ~0.18 and ~1.13.
...
9. **Validate temporal parameter scaling**: Ensure $T_{M}^*$ scaling correctly propagates to absolute timing parameters
10. **Assess parameter separation**: Verify clean separation between kinetic and temporal parameter inference
