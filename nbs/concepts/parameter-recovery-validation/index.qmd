...
**Mathematical Prohibition Against Parameter Averaging**: The posterior predictive distribution is **NOT** computed using averaged parameters:

$$\bar{\theta} = \frac{1}{S}\sum_{s=1}^S \theta^{(s)} \quad \text{(MATHEMATICALLY INCORRECT)}$$

Using $\bar{\theta}$ violates the fundamental definition of posterior predictive distributions and fails to capture parameter uncertainty propagation through the nonlinear piecewise dynamics.

**Correct Sample-wise Generation**: Each $\tilde{y}^{(s)}$ must be generated using the complete parameter vector $\theta^{(s)}$:
...
The mathematical definition of posterior predictive distributions explicitly requires sample-wise generation. **Parameter averaging is mathematically incorrect** because:

1. **Nonlinear dynamics**: The piecewise analytical solutions are nonlinear functions of parameters, so $f(\mathbb{E}[\theta]) \neq \mathbb{E}[f(\theta)]$
2. **Phase discontinuities**: Averaged temporal parameters may place cells in incorrect phases
3. **Uncertainty underestimation**: Averaging eliminates parameter uncertainty propagation
4. **Invalid inference**: Results do not represent valid Bayesian posterior predictive distributions
