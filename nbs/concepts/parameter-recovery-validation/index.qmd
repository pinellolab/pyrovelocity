...
**Characteristics**: Genes that start with baseline expression and become strongly activated during the process.

**Parameter specifications**:

- $R_{\text{on},i} > 3.0$ (strong fold-change during activation)
- $t^*_{0,\text{on}i} > 0$ and $t^*_{0,\text{on}i} < 0.4$ (activation occurs after observation start but early)
...
#### **2. Pre-Activation Patterns**

**Characteristics**: Genes that were already activated before observation began, showing high initial expression.

**Parameter specifications**:

- $R_{\text{on},i} > 2.0$ (moderate to strong fold-change)
- $t^*_{0,\text{on}i} < 0$ (activation onset before observation window)
- $\delta^*_i > |t^*_{0,\text{on}i}|$ (activation duration extends into observation window)

**Biological examples**: Early developmental genes, lineage commitment factors, stress response genes in ongoing stress

#### **3. Decay-Only Patterns**

**Characteristics**: Genes that start with high expression and gradually decrease without reactivation.

**Parameter specifications**:

- $R_{\text{on},i} > 1.5$ (moderate fold-change, but not observed during activation)
- $t^*_{0,\text{on}i} > T^*_M$ (activation onset beyond observation window) OR
- $t^*_{0,\text{on}i} < 0$ and $\delta^*_i < |t^*_{0,\text{on}i}|$ (activation ended before observation)

**Biological examples**: Maternal transcripts during development, pluripotency factors during differentiation

#### **4. Transient Activation Patterns**
**Characteristics**: Genes that show brief, pulse-like activation followed by return to baseline.

**Parameter specifications**:

- $R_{\text{on},i} > 2.0$ (moderate to strong fold-change)
- $t^*_{0,\text{on}i} > 0$ and $t^*_{0,\text{on}i} < 0.5$ (activation during observation)
- $\delta^*_i < 0.4$ (brief activation duration)

**Biological examples**: Immediate early genes, signaling molecules, transient developmental regulators

#### **5. Sustained Activation Patterns**
**Characteristics**: Genes that become activated and remain highly expressed throughout most of the process.

**Parameter specifications**:

- $R_{\text{on},i} > 2.0$ (strong fold-change)
- $t^*_{0,\text{on}i} > 0$ and $t^*_{0,\text{on}i} < 0.3$ (early activation onset)
- $\delta^*_i > 0.5$ (long activation duration)
...
# Prior hyperparameters (updated for constrained parameterization with Normal onset times)
priors = {
    ...
    'delta_star': {'loc': -0.8, 'scale': 0.45},  # log(0.45) [calibrated for balanced patterns] (LogNormal)
}

# Calculate constraint feasibility for each pattern
patterns = {
    'activation': [
        ('R_on', '>', 3.0), ('t_on_star', '>', 0.0),
        ('t_on_star', '<', 0.4), ('delta_star', '>', 0.3)
    ],
    'pre_activation': [
        ('R_on', '>', 2.0), ('t_on_star', '<', 0.0),
        ('delta_star', '>', 0.3)  # Simplified constraint
    ],
    'transient': [
        ('R_on', '>', 2.0), ('t_on_star', '>', 0.0),
        ('t_on_star', '<', 0.5), ('delta_star', '<', 0.4)
    ],
    'sustained': [
        ('R_on', '>', 2.0), ('t_on_star', '>', 0.0),
        ('t_on_star', '<', 0.3), ('delta_star', '>', 0.5)
    ]
}
...
The probabilistic graphical model extends Study 2 by adding gene-specific piecewise activation parameters while maintaining the hierarchical latent time structure. For visual clarity, we use $\theta_i$ to represent the vector of piecewise activation parameters $(\gamma^*_i, R_{\text{on},i}, t^*_{0,\text{on}i})$ and $\delta_i$ to represent the activation duration parameter. Note that $\alpha^*_{\text{off}i} = 1.0$ is fixed and not shown as a random variable.
...
- $\theta_i$ represents the vector of piecewise activation parameters $(\gamma^*_i, R_{\text{on},i}, t^*_{0,\text{on}i})$
...
The priors are designed to encode biologically reasonable parameter ranges while eliminating dimensional redundancy:
...
\begin{align}
    ...
    t^*_{0,\text{on}i} &\sim \text{Normal}(0.5, 0.8^2) \quad \text{(activation onset time, allows negatives, calibrated)} \\
    \delta^*_i &\sim \text{LogNormal}(\log(0.45), 0.45^2) \quad \text{(activation duration, calibrated for balanced patterns)}
\end{align}
...
- **$R_{\text{on},i} \sim \text{LogNormal}(\log(2.5), 0.4^2)$**: Centers fold-change at 2.5× during activation, with 95% between ~1.5 and ~4.2. This represents meaningful upregulation while avoiding extreme values that could cause numerical issues.

- **$t^*_{0,\text{on}i} \sim \text{Normal}(0.2, 0.6^2)$**: Centers activation onset at 20% through the process timeline, with 95% between approximately -1.0 and +1.4. **Negative values** represent genes already activated before observation began, enabling modeling of pre-activation scenarios.

- **$\delta^*_i \sim \text{LogNormal}(\log(0.37), 0.35^2)$**: Centers activation duration at 37% of the process timeline, with 95% between ~0.18 and ~0.76. This distribution was optimized to support both transient (< 40%) and sustained (> 50%) activation patterns with sufficient feasibility.
...
- **Activation genes**: Transition from baseline to high expression during observation
- **Pre-activation genes**: Already activated before observation, showing high initial expression
...
1. **Recover constrained piecewise parameters**: $\gamma^*_i$, $R_{\text{on},i}$, $t^*_{0,\text{on}i}$, $\delta^*_i$
2. **Handle pre-activation scenarios**: Correctly infer negative onset times and their biological implications
3. **Distinguish enhanced expression patterns**: Classify genes across all five patterns including pre-activation
4. **Maintain hierarchical time inference**: Recover population-level temporal parameters
...
8. **Assess dimensional consistency**: Verify elimination of parameter redundancy issues
9. **Numerical robustness**: Maintain stability across the $\gamma^*_i = 1$ boundary
...
# If we wanted to generate data from a specific parameter set, we could
# first sample some parameters from the prior
...
#     n_cells=200
# )
# and then use those to generate data from a single sample
# Take first parameter sample for data generation
# single_param_sample = {key: value[:1] for key, value in prior_parameter_samples.items()}
# And then set
# model.generate_predictive_samples(samples=single_param_sample, ...)
# But here we want to see the full breadth of what the prior can produce
# so we generate data from multiple parameter sets in a single call
...
We generate synthetic datasets representing each of the four gene expression patterns using the new streamlined validation API:
...
Now we demonstrate the complete parameter recovery validation workflow using the finalized PyroVelocity API.
...
The validation results provide comprehensive assessment of parameter recovery performance.
...
print(f"\n📋 Detailed Results:")
validation_results.display_table()
```
...
from pyrovelocity.models.modular.validation import plot_validation_summary
...
print(f"� Visualization saved to reports/docs/parameter_recovery_validation.png")
```

#### Validation Study 3 Summary
