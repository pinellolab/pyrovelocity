...
\begin{align}
T_{\max} &\sim \text{Gamma}(\alpha_{T}, \beta_{T}) \quad \text{(process duration)} \\
t^*_j &\sim \text{Uniform}(0, T_{\max}) \quad \text{(cell temporal positions)} \\
t^*_{0,\text{on}i} &\sim \text{Normal}(\mu_{on}, \sigma_{on}^2) \quad \text{(absolute activation onset time)} \\
...
- $t^*_{0,\text{on}i} < 0.0$ (absolute activation onset before observation window)
...
- $0 < t^*_{0,\text{on}i} < 0.3 \mathbb{E}[T_{\max}]$ (early activation within process duration)
- $\delta^*_i < 0.4 \mathbb{E}[T_{\max}]$ (short activation duration relative to process)
...
- $0 < t^*_{0,\text{on}i} < 0.3 \mathbb{E}[T_{\max}]$ (early activation within process duration)
- $\delta^*_i > 0.5 \mathbb{E}[T_{\max}]$ (long activation duration relative to process)
...
The process duration $T_{\max}$ determines the temporal scope of observable dynamics and requires careful hyperparameter selection based on biological expectations and experimental context.

#### **Dynamical Range and Steady-State Achievement**

In the dimensionless framework where the reference rate is 1, the process duration $T_{\max}$ determines what fraction of steady-state dynamics will be observable:

$$\text{Steady-state approach} = 1 - e^{-T_{\max}}$$
...
$$T_{\max} \sim \text{Gamma}(5.0, 1.0) \quad \text{(mean = 5, variance = 5)}$$
...
$$T_{\max} \sim \text{Gamma}(1.0, 0.2) \quad \text{(mean = 5, variance = 25)}$$
...
$$T_{\max} \sim \text{Gamma}(1.0, 1.0) \quad \text{(mean = 1, variance = 1, heavy tail)}$$
...
With independent parameterization, gene timing parameters should be calibrated to $T_{\max}$ expectations:

\begin{align}
t^*_{0,\text{on}i} &\sim \text{Normal}(0.3 \mathbb{E}[T_{\max}], \sigma_{on}^2) \quad \text{(expect activation at 30% of process)} \\
\delta^*_i &\sim \text{LogNormal}(\log(0.4 \mathbb{E}[T_{\max}]), \sigma_{\delta}^2) \quad \text{(expect 40% duration activation)}
\end{align}
...
# Assuming T_max ~ Gamma(5.0, 1.0) with E[T_max] = 5.0
E_T_max = 5.0
priors = {
    'R_on': {'loc': 0.916, 'scale': 0.4},                           # log(2.5), fold-change (LogNormal)
    't_on_star': {'loc': 0.3 * E_T_max, 'scale': 0.8},             # Normal(1.5, 0.8²), absolute onset
    'delta_star': {'loc': np.log(0.4 * E_T_max), 'scale': 0.45},   # log(2.0), absolute duration (LogNormal)
}
...
patterns = {
    ...
    'transient': [
        ...
        ('t_on_star', '<', 0.3 * E_T_max),         # Early enough to complete cycle
        ('delta_star', '<', 0.4 * E_T_max),        # Short pulse duration
        ('R_on', '>', 2.0),                        # Sufficient fold change to observe
    ],
    'sustained': [
        ('t_on_star', '>', 0.0),                    # Activation within observation window
        ('t_on_star', '<', 0.3 * E_T_max),         # Early activation onset
        ('delta_star', '>', 0.5 * E_T_max),        # Long activation duration
...
- $\Delta_i$ represents the vector of **independent temporal** gene parameters $(t^*_{0,\text{on}i}, \delta^*_i)$
...
- $T_{\max}$ determines the process duration and constrains cell temporal positions via $t^*_j \sim \text{Uniform}(0, T_{\max})$
...
- **Pre-activation**: $t^*_{0,\text{on}i} < 0.0$ and $R_{\text{on},i} > 2.0$
- **Transient**: $0 < t^*_{0,\text{on}i} < 0.3 \mathbb{E}[T_{\max}]$, $\delta^*_i < 0.4 \mathbb{E}[T_{\max}]$, and $R_{\text{on},i} > 2.0$
- **Sustained**: $0 < t^*_{0,\text{on}i} < 0.3 \mathbb{E}[T_{\max}]$, $\delta^*_i > 0.5 \mathbb{E}[T_{\max}]$, and $R_{\text{on},i} > 2.0$
...
- **$T_{\max} \sim \text{Gamma}(\alpha_{T}, \beta_{T})$**: Process duration with hyperparameters chosen based on experimental context (conservative: Gamma(5,1), permissive: Gamma(1,0.2), skeptical: Gamma(1,1)).

- **$t^*_{0,\text{on}i} \sim \text{Normal}(\mu_{on}, \sigma_{on}^2)$**: Absolute activation onset time, typically centered around $0.3 \mathbb{E}[T_{\max}]$ to expect activation at 30% of process duration. **Negative values** enable pre-activation scenarios where genes were already activated before observation began.

- **$\delta^*_i \sim \text{LogNormal}(\mu_{\delta}, \sigma_{\delta}^2)$**: Absolute activation duration, typically centered around $0.4 \mathbb{E}[T_{\max}]$ to expect 40% duration activation. This enables both transient (short duration) and sustained (long duration) activation patterns.

**Pattern classification implications:**

- **Pre-activation** ($t^*_{0,\text{on}i} < 0$): Probability depends on $\mu_{on}$ and $\sigma_{on}$ choices
- **Transient** ($0 < t^*_{0,\text{on}i} < 0.3 \mathbb{E}[T_{\max}]$ and $\delta^*_i < 0.4 \mathbb{E}[T_{\max}]$): Balanced probability for observable complete cycles
- **Sustained** ($0 < t^*_{0,\text{on}i} < 0.3 \mathbb{E}[T_{\max}]$ and $\delta^*_i > 0.5 \mathbb{E}[T_{\max}]$): Balanced probability for long-duration activation
...
where $\theta$ represents the complete parameter vector including all gene-specific parameters $(\gamma^*_i, R_{\text{on},i}, t^*_{0,\text{on}i}, \delta^*_i, U_{0i})$, independent time parameters $(T_{\max}, t^*_j)$, and cell-specific capture efficiencies $(\lambda_j)$.
...
&\quad p(T_{\max}) \, p(\theta_{\text{gene}}) \, p(\theta_{\text{cell}}) \, p(\theta_{\text{tech}}) \, dT_{\max} \, d\theta_{\text{gene}} \, d\theta_{\text{cell}} \, d\theta_{\text{tech}} \label{eq-independent-integration}
\end{align}
...
1. **Sample process duration**:
   \begin{align}
   {T_{\max}}^{(s)} &\sim \text{Gamma}(\alpha_{T}, \beta_{T})
   \end{align}

2. **Sample cell-specific temporal positions**:
   \begin{align}
   {t^{*}_j}^{(s)} &\sim \text{Uniform}(0, {T_{\max}}^{(s)}) \quad \text{for } j \in \{1, \ldots, N\}
   \end{align}
...
3. **Sample gene-specific parameters**:
   ...
   {t^*_{0,\text{on}i}}^{(s)} &\sim \text{Normal}(\mu_{on}, \sigma_{on}^2) \\
...
1. **Process duration uncertainty**: Uncertainty in $T_{\max}$ affects the temporal scope of the biological process and constrains cell temporal positions
...
4. **Gene-specific temporal pattern uncertainty**: Prior uncertainty in $t^*_{0,\text{on}i}, \delta^*_i$ affects when activation occurs and how long it lasts in absolute time
...
- **Independent temporal gene parameters**: $t^*_{0,\text{on}i} \sim \text{Normal}(\mu_{on}, \sigma_{on}^2)$, $\delta^*_i \sim \text{LogNormal}(\mu_{\delta}, \sigma_{\delta}^2)$
- **Characteristic scales**: $U_{0i} \sim \text{LogNormal}(\mu_{U_0}, \sigma_{U_0}^2)$
- **Process duration**: $T_{\max} \sim \text{Gamma}(\alpha_{T}, \beta_{T})$
- **Cell-specific times**: $t^*_j \sim \text{Uniform}(0, T_{\max})$
...
2. **Temporal parameter uncertainty**: Uncertainty in $t^*_{0,\text{on}i}$ and $\delta^*_i$ affects when activation occurs and how long it lasts in absolute time
3. **Process duration uncertainty**: Uncertainty in $T_{\max}$ affects the temporal scope and constrains cell temporal positions
...
1. **Recover independent piecewise parameters**: $\gamma^*_i$, $R_{\text{on},i}$, $t^*_{0,\text{on}i}$, $\delta^*_i$
2. **Maintain process duration inference**: Recover $T_{\max}$ and cell temporal positions $t^*_j$
...
8. **Assess parameter separation**: Verify clean separation between kinetic ($\gamma^*_i$, $R_{\text{on},i}$) and temporal ($t^*_{0,\text{on}i}$, $\delta^*_i$) parameter inference
