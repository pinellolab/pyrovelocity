...
This piecewise framework enables representation of diverse gene expression patterns:

1. **Activation patterns**: Low $\alpha^*_{\text{off}i}$, early $t^*_{0,\text{on}i}$, high $\alpha^*_{\text{on}i}$
2. **Decay-only patterns**: High $\alpha^*_{\text{off}i}$, $t^*_{0,\text{on}i} > T^*_M$ (onset beyond observation window)
3. **Transient activation**: Early $t^*_{0,\text{on}i}$, short $\delta^*_i$, moderate $\alpha^*_{\text{on}i}$
4. **Sustained activation**: Early $t^*_{0,\text{on}i}$, long $\delta^*_i$, high $\alpha^*_{\text{on}i}$
