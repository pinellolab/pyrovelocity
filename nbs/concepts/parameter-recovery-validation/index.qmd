...
**Identifiability proof**: For any $\lambda \neq 1$:
$$\mathcal{L}(T_{\max}, \{t^*_j\}, \{t^*_{0,\text{on}i}\}, \{\delta^*_i\}) \neq \mathcal{L}(\lambda T_{\max}, \{\lambda t^*_j\}, \{t^*_{0,\text{on}i}\}, \{\delta^*_i\})$$

The fixed anchor $t^*_1 = 0$ cannot be scaled, and gene parameters are independent of $T_{\max}$, breaking the symmetry.
...
**Pattern classification framework**: The three expression patterns are defined by constraints on the absolute temporal parameters:

- **Pre-activation**: $t^*_{0,\text{on}i} < 0.0$ and $R_{\text{on},i} > 2.0$
- **Transient**: $0.0 < t^*_{0,\text{on}i} < 2.5$, $\delta^*_i < 2.0$, and $R_{\text{on},i} > 2.0$
- **Sustained**: $0.0 < t^*_{0,\text{on}i} < 1.5$, $\delta^*_i > 2.5$, and $R_{\text{on},i} > 2.0$
...
**Stick-breaking cell time coordinates:**

The cell times are sampled using a stick-breaking process that ensures proper ordering and eliminates scaling symmetry:
...
This construction automatically ensures $0 = t^*_1 < t^*_2 < \cdots < t^*_N = T_{\max}$ and eliminates the need for clipping operations.
...
2. **Sample stick-breaking variables and construct cell times**:
   \begin{align}
   {\xi_j}^{(s)} &\sim \text{Beta}(1, N-j) \quad \text{for } j \in \{2, \ldots, N-1\} \\
   {t^*_1}^{(s)} &= 0 \\
   {t^*_N}^{(s)} &= {T_{\max}}^{(s)} \\
   {t^*_j}^{(s)} &= {t^*_{j-1}}^{(s)} + {\xi_{j-1}}^{(s)}({T_{\max}}^{(s)} - {t^*_{j-1}}^{(s)})
   \end{align}
