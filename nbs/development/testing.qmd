...
| likelihood_model | Poisson likelihood model defines distributions | ✅ Implemented | Verify that the PoissonLikelihoodModel defines appropriate Poisson distributions |
...
| likelihood_model | Likelihood model with scaling factors | ✅ Implemented | Verify that the likelihood model correctly incorporates scaling factors |
| likelihood_model | Likelihood model with zero counts | ✅ Implemented | Verify that the likelihood model handles zero counts gracefully |
| likelihood_model | Likelihood model with Pyro plates | ✅ Implemented | Verify that the likelihood model correctly uses Pyro plates for batch dimensions |
| observation_model | Standard observation model transforms data | ✅ Implemented | Verify that the StandardObservationModel correctly transforms input data |
| observation_model | Observation model computes size factors | ✅ Implemented | Verify that the observation model correctly computes size factors |
| observation_model | Observation model handles missing values | ✅ Implemented | Verify that the observation model handles missing values gracefully |
| observation_model | Observation model with normalization | ✅ Implemented | Verify that the observation model correctly normalizes data |
| observation_model | Observation model preserves data integrity | ✅ Implemented | Verify that the observation model preserves biological signal and doesn't introduce artifacts |
| guide_model | AutoGuide factory creates guide | ✅ Implemented | Verify that the AutoGuideFactory creates an appropriate guide |
...
| guide_model | Guide with parameter initialization | ✅ Implemented | Verify that the guide correctly initializes parameters with specified scale |
| guide_model | Guide with different variational families | ✅ Implemented | Verify that the guide supports different variational families |
| guide_model | Guide with custom initialization | ✅ Implemented | Verify that the guide supports custom initialization functions |
...
PyroVelocity uses feature files to describe the expected behavior of each component. Here are examples from different components:
...
Step definition files connect feature files to actual test code. They are organized by component in the `integration/` directory. PyroVelocity has implemented step definitions for all the major components:

- `test_dynamics_model_steps.py`: Implements steps for testing dynamics models
- `test_prior_model_steps.py`: Implements steps for testing prior models
- `test_likelihood_model_steps.py`: Implements steps for testing likelihood models
- `test_observation_model_steps.py`: Implements steps for testing observation models
- `test_guide_model_steps.py`: Implements steps for testing inference guides
- `test_model_steps.py`: Implements steps for testing the full PyroVelocityModel

Additionally, there's a `test_component_integration.py` file that tests the interactions between different components to ensure they work correctly together.

Here's an example of a step definition file:
...
In addition to the BDD step definitions, PyroVelocity includes component integration tests that verify the interactions between different components:

```python
# src/pyrovelocity/tests/models/modular/integration/test_component_integration.py
