...
graph TB
    ...
    Main -->|uses| Validation["Validation Framework"]
    ...
    Validation -->|contains| ParameterRecovery["Parameter Recovery"]
    Validation -->|contains| ModelComparison["Model Comparison"]
    Validation -->|contains| ValidationMetrics["Validation Metrics"]
    ...
    class Validation,ParameterRecovery,ModelComparison,ValidationMetrics validation;
```

### Core Components {#sec-overview-core-components}
...
9. **Validation Framework**: Provides tools for model validation and comparison
   - Parameter Recovery: Validates model's ability to recover known parameters from synthetic data
   - Model Comparison: Compares different model implementations and configurations
   - Validation Metrics: Computes metrics for quantifying validation performance
...
| ParameterRecovery | generate_synthetic_data | Generates synthetic data with known parameters | FN-03 |
| ParameterRecovery | validate_parameter_recovery | Validates model's ability to recover known parameters | FN-03 |
| ModelComparison | compare_parameters | Compares parameter estimates between implementations | FN-04 |
| ValidationMetrics | compute_parameter_metrics | Computes metrics for parameter recovery validation | FN-04 |
...
| AD-06 | Parameter Recovery Validation | Validate model's ability to recover known parameters from synthetic data before applying to real data | Direct comparison to legacy implementation only | Ensures model is internally consistent and capable of recovering parameters, prerequisite for robust model comparison |
...
**Related Algorithm Specification:** ALG-03 (Stochastic Variational Inference)
...
```python
def train_svi(self, adata: AnnData, **kwargs) -> Dict[str, Any]:
    ...
    return {"losses": losses, "guide": guide, "posterior_samples": guide.get_posterior()}
```

**Algorithm ID:** ALG-IMP-02

**Algorithm Name:** Markov Chain Monte Carlo for RNA Velocity

**Related Algorithm Specification:** ALG-04 (Markov Chain Monte Carlo Inference)

**Implementation Approach:** NumPyro MCMC with NUTS

**Algorithm Structure:**

```python
def train_mcmc(self, adata: AnnData, **kwargs) -> Dict[str, Any]:
    # Extract data from AnnData
    data = prepare_anndata(adata)
    u_obs = data["u"]
    s_obs = data["s"]
    ...
    # Create MCMC kernel
    kernel = numpyro.infer.NUTS(self.forward)
    ...
    # Run MCMC
    mcmc.run(jax.random.PRNGKey(0), u_obs=u_obs, s_obs=s_obs)

    # Get diagnostics
    diagnostics = {
        "r_hat": mcmc.get_diagnostics()["r_hat"],
        "effective_sample_size": mcmc.get_diagnostics()["effective_sample_size"],
        "acceptance_rate": mcmc.get_diagnostics()["acceptance_rate"],
        "divergences": mcmc.get_diagnostics()["divergences"]
    }
...
**Method Name:** Parameter Recovery Validation

**Related Function:** FN-03 (Parameter Recovery Validation)

**Description:** Validates model's ability to recover known parameters from synthetic data

**Method Details:**

- Parameter generation: Samples parameter sets from prior distributions
- Synthetic data generation: Creates synthetic data using known parameters
- Inference: Performs inference on synthetic data to estimate parameters
- Evaluation: Compares estimated parameters to original known values
- Metrics computation: Calculates recovery accuracy, precision, and coverage metrics

**Implementation Details:**

- Libraries/frameworks used: PyTorch, Pyro, NumPy, SciPy
- Computational complexity: O(num_parameter_sets * num_samples * max_sample_size * num_genes)
- Memory requirements: O(num_parameter_sets * num_samples * max_sample_size * num_genes)
- Parallelization strategy: Parallel processing across parameter sets and samples
...
**Interface Name:** ParameterRecoveryValidator Protocol
...
```python
@runtime_checkable
class ParameterRecoveryValidator(Protocol):
    ...
    def validate_parameter_recovery(
        ...
        **kwargs
        ...
        """
        Validate model's ability to recover known parameters.
        """
        ...
```
...
| Parameter | Type | Description | Default Value | Constraints |
|-----------|------|-------------|---------------|-------------|
| model_config | Dict[str, Any] | Model configuration | None | Must specify model components |
| prior_config | Dict[str, Any] | Prior distribution configuration | None | Must specify prior distributions for all parameters |
| num_cells | int | Number of cells in synthetic data | None | > 0 |
| num_genes | int | Number of genes in synthetic data | None | > 0 |
| synthetic_data | Dict[str, Any] | Synthetic data with known parameters | None | Must contain u_obs, s_obs, true_parameters |

**Return Values:**

| Return Value | Type | Description | Constraints |
|--------------|------|-------------|-------------|
| synthetic_data | Dict[str, Any] | Synthetic data with known parameters | Must contain u_obs, s_obs, true_parameters |
| validation_results | Dict[str, Any] | Validation metrics and results | Must contain recovery_metrics, coverage_metrics |

**Error Handling:**

- Missing parameters: Raises ValueError with descriptive message
- Invalid parameters: Validates parameters and raises ValueError for invalid values
- Inference failure: Provides diagnostic information for inference failures
...
graph TB
    ...
    Main -->|contains| Observations["Observation Models"]
    Main -->|contains| Guides["Inference Guides"]
    Main -->|contains| MCMC["MCMC Models"]
    Main -->|contains| Registry["Registry System"]
    Main -->|contains| Factory["Factory System"]
    Main -->|contains| Validation["Validation Framework"]
    ...
    Observations -->|implements| ObservationsInterface["ObservationModel Protocol"]
    Guides -->|implements| GuidesInterface["InferenceGuide Protocol"]
    MCMC -->|implements| MCMCInterface["MCMCModel Protocol"]
    ...
    class Main main;
    class Dynamics,Priors,Likelihoods,Observations,Guides,MCMC component;
    class DynamicsInterface,PriorsInterface,LikelihoodsInterface,ObservationsInterface,GuidesInterface,MCMCInterface interface;
    class Registry,Factory system;
    class Validation,ParameterRecovery,ModelComparison,ValidationMetrics validation;
```
...
- **LegacyDynamicsModel**: Compatibility implementation matching the legacy model
- **NonlinearDynamicsModel**: Implementation supporting nonlinear dynamics with numerical integration
...
classDiagram
    ...
    class NonlinearDynamicsModel {
        ...
        +forward(context) Dict
        +steady_state(alpha, beta, gamma) Tuple
        +solve_ode(initial_state, params, times) Tensor
    }

    DynamicsModel <|.. StandardDynamicsModel : implements
    DynamicsModel <|.. LegacyDynamicsModel : implements
    DynamicsModel <|.. NonlinearDynamicsModel : implements
```
...
#### MCMC Models {#sec-mcmc-models}

MCMC models implement Markov Chain Monte Carlo sampling for Bayesian inference.

- **NUTSModel**: Implements the No U-Turn Sampler for MCMC
- **HMCModel**: Implements Hamiltonian Monte Carlo for MCMC

```{mermaid}
classDiagram
    class MCMCModel {
        <<Protocol>>
        +sample(model, data, num_samples, num_chains) Dict
    ...
    class NUTSModel {
        ...
        +sample(model, data, num_samples, num_chains) Dict
        +get_diagnostics() Dict
        +compute_r_hat() Tensor
        +compute_effective_sample_size() Tensor
    }
    ...
    MCMCModel <|.. NUTSModel : implements
    MCMCModel <|.. HMCModel : implements
```

#### Validation Framework {#sec-core-validation-framework}

The validation framework provides tools for model validation and comparison.

- **ParameterRecoveryValidator**: Validates model's ability to recover known parameters from synthetic data
- **ModelComparisonFramework**: Compares different model implementations and configurations
- **ValidationMetrics**: Computes metrics for quantifying validation performance

```{mermaid}
classDiagram
    class ParameterRecoveryValidator {
        <<Protocol>>
        +generate_synthetic_data(model_config, prior_config, num_cells, num_genes) Dict
        +validate_parameter_recovery(model_config, synthetic_data) Dict
    }

    class ModelComparisonFramework {
        <<Protocol>>
        +compare_models(models, data, metrics) Dict
        +rank_models(comparison_results, criteria) Dict
    }

    class ValidationMetrics {
        <<Protocol>>
        +compute_parameter_metrics(true_params, estimated_params) Dict
        +compute_coverage_metrics(true_params, credible_intervals) Dict
        +compute_model_comparison_metrics(models, data) Dict
    }

    class StandardParameterRecoveryValidator {
        +generate_synthetic_data(model_config, prior_config, num_cells, num_genes) Dict
        +validate_parameter_recovery(model_config, synthetic_data) Dict
        +compute_validation_metrics(true_params, estimated_params) Dict
    }

    class StandardModelComparisonFramework {
        +compare_models(models, data, metrics) Dict
        +rank_models(comparison_results, criteria) Dict
        +visualize_comparison(comparison_results) Dict
    }

    ParameterRecoveryValidator <|.. StandardParameterRecoveryValidator : implements
    ModelComparisonFramework <|.. StandardModelComparisonFramework : implements
```
...
# AnnData Integration {#sec-anndata-integration}

PyroVelocity integrates with AnnData objects following these principles:

1. **Mutation Tracking**: All methods that modify AnnData objects must track and document mutations using the `print_anndata`, `anndata_string`, and `print_string_diff` utilities.

2. **Return Values**: Methods that modify AnnData objects must return the modified object to enable method chaining and make mutations explicit.

3. **Required Parameters**: AnnData parameters should be required (not optional) when they are essential for method functionality.

4. **Shape Validation**: Array shapes should be validated using jaxtyping rather than complex shape handling code.

5. **Explicit Documentation**: Methods that modify objects must explicitly document this behavior in their docstrings.

6. **Distinction Between Model State and External Data**: While the model state follows an immutable pattern (using `ModelState` and `with_state()`), external data structures like AnnData objects can be mutated in-place for efficiency. This distinction is important because:
   - Model state must remain consistent and thread-safe
   - AnnData objects are large and copying them is expensive
   - In-place mutation of AnnData is a common pattern in the single-cell ecosystem

These principles ensure that mutations to AnnData objects are explicit, traceable, and well-documented, making the code more maintainable and reducing the risk of bugs while maintaining performance with large datasets.
