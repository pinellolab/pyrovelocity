...
Installation should take less than **10 minutes**. If you are running in [Google Colab][colab-more-memory], then even when running with the `PYROVELOCITY_TESTING_FLAG` set to `True`,
if you have [access to *High-RAM* or *T4 GPU* runtimes][colab-more-memory] this can significantly speed up even this testing of the notebook.
...
This section involves checking if the notebook is running in colab, in which case it is certain that you will need to install or reinstall pyrovelocity.
