#!/usr/bin/env python
"""
Pancreas Dataset Validation Script for PyroVelocity.
...
The script uses the downsampled pancreas dataset and provides detailed
text-based reports of the validation results.
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Pancreas Dataset Validation Script for PyroVelocity"
    )
    parser.add_argument(
        "--max-epochs",
        type=int,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="pancreas_validation_results",
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Number of cells to use from the dataset",
    )
    parser.add_argument(
        "--n-genes",
        type=int,
        default=100,
        help="Number of genes to use from the dataset",
    )
    parser.add_argument(
        "--learning-rate",
        type=float,
...
def create_synthetic_data(n_cells=50, n_genes=20, seed=42):
    """Create synthetic data for testing when pancreas dataset is not available."""
...
def prepare_pancreas_data(n_cells=100, n_genes=100, seed=42):
    """Load and prepare pancreas data for validation."""
    # Load data
    print("Loading pancreas data...")
    try:
        adata = pancreas()
        print(f"Loaded dataset with {adata.shape[0]} cells and {adata.shape[1]} genes")

        # Subset data for faster validation
        print(f"Subsetting data to {n_cells} cells and {n_genes} genes...")
        
        # Set random seed for reproducibility
        np.random.seed(seed)
        
        # Randomly select cells and genes
        cell_indices = np.random.choice(adata.shape[0], min(n_cells, adata.shape[0]), replace=False)
        gene_indices = np.random.choice(adata.shape[1], min(n_genes, adata.shape[1]), replace=False)
        
        adata = adata[cell_indices, gene_indices].copy()
        ...
        adata = create_synthetic_data(n_cells=n_cells, n_genes=n_genes, seed=seed)
...
def generate_summary_report(validation_results, args, output_dir):
    ...
    with open(report_path, "w") as f:
        f.write("PyroVelocity Pancreas Validation Summary Report\n")
        f.write("===========================================\n\n")
        ...
        f.write(f"Dataset size: {args.n_cells} cells, {args.n_genes} genes\n")
...
def main():
    ...
    adata = prepare_pancreas_data(n_cells=args.n_cells, n_genes=args.n_genes, seed=args.seed)
