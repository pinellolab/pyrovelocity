...
from pyro.distributions import LogNormal, Normal, Poisson, Bernoulli, NegativeBinomial, Beta, Binomial, Categorical, Gamma, Delta, Multinomial, InverseGamma, Dirichlet
import pyro.distributions as dist
from typing import Optional, Union, Tuple, Literal
from .utils import mRNA, debug, tau_inv, ode_mRNA
...
class LogNormalModel(PyroModule):
    def __init__(
        ...
        if self.likelihood == "NB":
            self.u_px_r = PyroParam(torch.ones(self.num_genes), constraint=positive, event_dim=0)
            self.s_px_r = PyroParam(torch.ones(self.num_genes), constraint=positive, event_dim=0)
    ...
    def alpha(self):
        ...
        return LogNormal(self.zero, self.one*0.1).expand((self.num_genes, )).to_event(1).mask(False)
    ...
    def latent_time(self):
        if self.shared_time:
            if self.plate_size == 2:
                return Normal(self.zero, self.one*0.1).mask(self.include_prior) # with shared cell_time
            else:
                return Normal(self.zero, self.one*0.1).expand((self.num_genes, )).to_event(1).mask(self.include_prior)  # with shared cell_time
        if self.plate_size == 2:
    ...
    def cell_time(self):
        if self.plate_size == 2:
            if self.shared_time:
                return Normal(self.zero, self.one) #.mask(self.include_prior) # mask=False generate the same estimation as initialization
    ...
    def cell_codebook(self):
        return Normal(self.zero, self.one / (10 * self.latent_factor_size)).expand((self.latent_factor_size, self.num_genes*2)).to_event(2).mask(self.include_prior)

    @PyroSample
    def cell_code(self):
        return Normal(self.zero, self.one).expand((self.latent_factor_size, )).to_event(1).mask(self.include_prior)
    ...
    def _get_fn_args_from_batch(tensor_dict):
        ...
        if "time_info" in tensor_dict:
            ...
            time_info = None
        return (u_obs, s_obs, u_log_library, s_log_library, u_log_library_mean, s_log_library_mean, u_log_library_scale, s_log_library_scale, ind_x, cell_state, time_info), {}

    def get_likelihood(self, ut, st, u_log_library=None, s_log_library=None, u_scale=None, s_scale=None, u_read_depth=None, s_read_depth=None, u_cell_size_coef = None, ut_coef = None, s_cell_size_coef = None, st_coef = None):
        ...
        if self.likelihood == 'NB':
            if self.correct_library_size:
                ...
                u_logits = ((ut + self.one*1e-6) * u_read_depth).log() - (self.u_px_r.exp() + self.one*1e-6).log()
                s_logits = ((st + self.one*1e-6) * s_read_depth).log() - (self.s_px_r.exp() + self.one*1e-6).log()
            else:
                ut = relu(ut)
                st = relu(st)
                ut = pyro.deterministic("ut", ut, event_dim=0)
            ...
            if self.correct_library_size:
                ...
                if self.correct_library_size == 'cell_size_regress':
                    ...
                    ut = torch.log(ut)
                    st = torch.log(st)
                    ut = torch.exp(ut_coef*ut +u_cell_size_coef*(-ut_sum + u_read_depth))
                    st = torch.exp(st_coef*st +s_cell_size_coef*(-st_sum + s_read_depth))
                else:
                    ut = ut / torch.sum(ut, dim=-1, keepdim=True)
                    st = st / torch.sum(st, dim=-1, keepdim=True)
                ut = pyro.deterministic("ut_norm", ut, event_dim=0)
            ...
            if u_scale is not None and s_scale is not None:
                u_dist = Normal(ut, u_scale)  # NOTE: add scale parameters significantly decrease ELBO
                s_dist = Normal(st, s_scale)
            else:
                u_dist = Normal(ut, self.one * 0.1)  # NOTE: add scale parameters significantly decrease ELBO
                s_dist = Normal(st, self.one * 0.1)
        elif self.likelihood == 'LogNormal':
            if u_scale is not None and s_scale is not None:
                u_dist = LogNormal((ut+self.one * 1e-6).log(), u_scale)  # NOTE: add scale parameters significantly decrease ELBO
                s_dist = LogNormal((st+self.one * 1e-6).log(), s_scale)
            else:
                u_dist = LogNormal(ut, self.one * 0.1)  # NOTE: add scale parameters significantly decrease ELBO
...
class VelocityModel(LogNormalModel):
    def __init__(
        ...
        **initial_values
        ...
        self.mask = initial_values.get('mask', torch.ones(self.num_cells, self.num_genes).bool())
    ...
    def model(
        ...
        cell_plate, gene_plate = self.create_plates(u_obs, s_obs, u_log_library, s_log_library, ind_x)
        ...
        with cell_plate, gene_plate, \
                poutine.mask(mask=pyro.subsample(self.mask.to(alpha.device), event_dim=0)):
            t = self.latent_time
            if self.shared_time:
                if self.t_scale_on:
            ...
            if self.latent_factor_operation == 'selection':
                velocity_genecellpair = pyro.sample("genecellpair_type", Bernoulli(p_velocity))
            if self.latent_factor == 'linear':
            ...
            if self.latent_factor_operation == 'selection':
                ...
                st = st
            u_dist, s_dist = self.get_likelihood(ut, st, u_log_library, s_log_library, u_scale, s_scale, u_read_depth, s_read_depth)
    ...
    def model2(
        ...
        cell_plate = self.create_plate(u_obs, s_obs, u_log_library, s_log_library, ind_x)
        alpha = self.alpha
        beta = self.beta
        gamma = self.gamma
        switching = self.switching
        ...
        with cell_plate: #, poutine.mask(mask=pyro.subsample(self.mask.to(alpha.device), event_dim=1)):
            t = self.latent_time
            u_inf, s_inf = mRNA(switching, self.zero, self.zero, alpha, beta, gamma)
            state = pyro.sample("cell_gene_state", Bernoulli(logits=t-switching).to_event(1)) == self.zero
            u0_vec = torch.where(state, self.zero, u_inf)
            s0_vec = torch.where(state, self.zero, s_inf)
            alpha_vec = torch.where(state, alpha, self.zero)
...
class AuxCellVelocityModel(VelocityModel):
    def forward(
        ...
        """ max plate = 2 with cell and gene plate """
        cell_plate, gene_plate = self.create_plates(u_obs, s_obs, u_log_library, s_log_library, u_log_library_loc, s_log_library_loc, u_log_library_scale, s_log_library_scale, ind_x)
        with gene_plate, poutine.mask(mask=self.include_prior):
            alpha = self.alpha
            beta = self.beta
            gamma = self.gamma
        ...
        with cell_plate: #, poutine.mask(mask=cell_gene_mask):
            ...
            with pyro.condition(data={"u": u_obs, "s": s_obs}):
                ut, st = self.generate_cell(
                    ...
                    t_scale=t_scale, gene_offset=gene_offset,
                    p_velocity=p_velocity, decoder_on=self.decoder_on,
                    u_read_depth=u_read_depth, s_read_depth=s_read_depth,
                    u0=u0, s0=s0)
        ...
        if self.num_aux_cells > 0:
            with pyro.contrib.autoname.scope(prefix="aux"):
                # TODO set self.num_aux_cells in self.__init__, 10-200
                aux_cell_plate = pyro.plate("aux_cell_plate", self.num_aux_cells, dim=cell_plate.dim)
                with aux_cell_plate:
    ...
    def generate_cell(
        ...
        if self.latent_factor == 'linear':
            cell_code = pyro.sample("cell_code", Normal(self.zero, self.one).expand((self.latent_factor_size, )).to_event(1).mask(self.include_prior))
        else:
            cell_code = None
        ...
        if self.shared_time:
            ...
            cell_time = pyro.sample("cell_time", LogNormal(self.zero, self.one).mask(self.include_prior)) # mask=False works for cpm and raw read count, count needs steady-state initialization
        ...
        with gene_plate:
            ...
            if not self.only_cell_times:
                if self.shared_time:
                    t = pyro.sample("latent_time", Normal(self.zero, self.one).mask(self.include_prior))
            ...
            if self.latent_factor == 'linear':
                ...
                regressor_s = softplus(regressor_output[..., self.num_genes:].squeeze() + s_pcs_mean)
            if self.latent_factor_operation == 'selection':
                ut = torch.where(cellgene_type == self.one, ut * u_scale / s_scale, softplus(regressor_u))
                ...
                ut = ut * u_scale / s_scale
                st = st
            u_dist, s_dist = self.get_likelihood(ut, st, u_log_library, s_log_library, u_scale, s_scale, u_read_depth=u_read_depth, s_read_depth=s_read_depth)
...
class TimeEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, *cat_list: int):
        ...
        q_m = self.act(self.mean_encoder(q)) # really big cell_time, this also generates reverse order with poisson
        return q_m
...
class BlockedKineticsModel(AuxCellVelocityModel):
    ...
    def get_time(self,
                 ...
                 dt_switching, u_inf, s_inf, u_offset=None, s_offset=None):
        ...
        if self.guide_type == 'auto':
            ...
            t = pyro.sample("cell_time", LogNormal(cell_time_loc, cell_time_scale))
        else:
    ...
    def forward(
        ...
        with gene_plate, poutine.mask(mask=self.include_prior):
            ...
            u0 = s0 = self.zero
            if self.add_offset:
                u_offset = pyro.sample("u_offset", LogNormal(self.zero, self.one))  # work with deeper network >=3 if start at zero
                s_offset = pyro.sample("s_offset", LogNormal(self.zero, self.one))
        ...
        with cell_plate:
            if self.guide_type == 'auto':
                t = self.get_time(u_scale, s_scale, alpha, beta, gamma, u_obs, s_obs, u0, s0, t0, dt_switching, u_inf, s_inf)
            elif self.guide_type == 'velocity_auto':
...
class MultiKineticsModel(VelocityModel):
    ...
    def forward(
        ...
        cell_plate, gene_plate, kinetics_plate = self.create_plates(u_obs, s_obs, u_log_library, s_log_library, u_log_library_loc, s_log_library_loc, u_log_library_scale, s_log_library_scale, ind_x, cell_state, time_info)
        ...
        with kinetics_plate, gene_plate, poutine.mask(mask=self.include_prior):
            ...
            dt_switching_k = pyro.sample("dt_switching", LogNormal(self.zero, self.one))
            ...
            if self.add_offset:
                u_offset_k = pyro.sample("u_offset", LogNormal(self.zero, self.one))
                s_offset_k = pyro.sample("s_offset", LogNormal(self.zero, self.one))
            else:
        ...
        u_inf_k, s_inf_k = mRNA(dt_switching_k, u_offset_k, s_offset_k, alpha_k, beta_k, gamma_k)
        ...
        with cell_plate:
            cell_time = pyro.sample("cell_time", LogNormal(self.zero, self.one).mask(self.include_prior))
        ...
        assert logits_k.shape == (kinetics_plate.size, cell_plate.subsample_size, gene_plate.size)
        ...
        with kinetics_plate, cell_plate, gene_plate:
            if self.guide_type == 'auto_t0_constraint' or self.guide_type == 'velocity_auto_t0_constraint':
                pyro.sample("time_constraint", Bernoulli(logits=cell_time-t0_k), obs=self.one)

        with kinetics_plate, cell_plate, gene_plate:
            state_k = pyro.sample("cell_gene_state", Bernoulli(logits=logits_k),
                                  infer={'enumerate': 'sequential'}) == self.zero
            ...
            if self.correct_library_size and (self.likelihood != 'Normal'):
                ...
                mse = (ut_k - u_obs)**2+(st_k - s_obs)**2
            assert mse.shape == (kinetics_plate.size, cell_plate.subsample_size, gene_plate.size)

        with cell_plate:
            mse = mse.permute(1, 2, 0)
            assert mse.shape == (cell_plate.subsample_size, gene_plate.size, kinetics_plate.size)
            mse = mse.mean(axis=-2, keepdims=True)

            assert mse.shape == (cell_plate.subsample_size, 1, kinetics_plate.size)
            kinetics_lineage = pyro.sample("mse", Categorical(logits=mse),
            ...
            with gene_plate:
                ...
                u_dist, s_dist = self.get_likelihood(ut, st, u_log_library, s_log_library, u_scale=None, s_scale=None, u_read_depth=u_read_depth, s_read_depth=s_read_depth)
                u = pyro.sample("u", u_dist, obs=u_obs)
                s = pyro.sample("s", s_dist, obs=s_obs)
...
class MultiKineticsModelDirichlet(VelocityModel):
    ...
    def get_time(self,
                 ...
                 alpha, beta, gamma,
                 u_obs, s_obs, u0, s0, t0,
                 dt_switching, u_inf, s_inf, u_read_depth=None, s_read_depth=None):
        u_ = u_obs
        s_ = s_obs
    ...
    def get_rna(self, alpha, beta, gamma,
                t, u0, s0, t0,
    ...
    def forward(
        ...
        with kinetics_plate, gene_plate:
            ...
            gamma_k = pyro.sample("gamma", LogNormal(self.zero, self.one))
            dt_switching_k = pyro.sample("dt_switching", LogNormal(self.zero, self.one))
            u_inf_k, s_inf_k = mRNA(dt_switching_k, self.zero, self.zero, alpha_k, beta_k, gamma_k)
        ...
        with cell_plate:
            kinetics_probs = pyro.sample("kinetics_prob", Dirichlet(self.one.new_ones(self.k)/self.k))
            assert kinetics_probs.shape == (cell_plate.subsample_size, 1, kinetics_plate.size), kinetics_probs.shape
            kinetics_lineage = pyro.sample("kinetics_lineage", Categorical(kinetics_probs),
                                           infer={'enumerate': 'parallel'})
                                           ##infer={'enumerate': 'sequential'})
            ...
            alpha = Vindex(alpha_k.transpose(kinetics_plate.dim, gene_plate.dim))[..., kinetics_lineage]
            beta = Vindex(beta_k.transpose(kinetics_plate.dim, gene_plate.dim))[..., kinetics_lineage]
            gamma = Vindex(gamma_k.transpose(kinetics_plate.dim, gene_plate.dim))[..., kinetics_lineage]
            switching = Vindex(switching_k.transpose(kinetics_plate.dim, gene_plate.dim))[..., kinetics_lineage]
            ##t0 = Vindex(t0_k.transpose(kinetics_plate.dim, gene_plate.dim))[..., kinetics_lineage]
            u_inf = Vindex(u_inf_k.transpose(kinetics_plate.dim, gene_plate.dim))[..., kinetics_lineage]
            s_inf = Vindex(s_inf_k.transpose(kinetics_plate.dim, gene_plate.dim))[..., kinetics_lineage]

            if self.add_offset:
                u0 = Vindex(u0_k.transpose(kinetics_plate.dim, gene_plate.dim))[..., kinetics_lineage]
                s0 = Vindex(s0_k.transpose(kinetics_plate.dim, gene_plate.dim))[..., kinetics_lineage]
            #print(s_inf.shape)
        ...
        with cell_plate, gene_plate:
            ...
            ut, st = self.get_rna(alpha, beta, gamma, t, u0, s0, t0, switching, u_inf, s_inf)
            u_dist, s_dist = self.get_likelihood(ut*u_scale/s_scale, st, u_log_library, s_log_library, None, None, u_read_depth=u_read_depth, s_read_depth=s_read_depth)
            u = pyro.sample("u", u_dist, obs=u_obs)
            s = pyro.sample("s", s_dist, obs=s_obs)
...
class MultiKineticsMultiSwitchModel(VelocityModel):
    ...
    def forward(
        ...
        with cell_plate:
            ...
            s_read_depth = pyro.sample("s_read_depth", LogNormal(s_log_library, s_log_library_scale))
            if self.guide_type == 'auto_t0_constraint' or self.guide_type == 'velocity_auto_t0_constraint':
                pyro.sample("time_constraint", Bernoulli(logits=t-t0), obs=self.one) # start point constraint
                with gene_plate: # end point constraint
                    pyro.sample("time_constraint2", Bernoulli(logits=switching_k[-1]-t), obs=self.one)

        with cell_plate, gene_plate:
            kinetics_logits = switching_k.transpose(kinetics_plate.dim, gene_plate.dim).unsqueeze(-2)
            assert kinetics_logits.shape == (1, gene_plate.size, kinetics_plate.size)
            kinetics_logits = t.unsqueeze(-1) - kinetics_logits
            assert kinetics_logits.shape == (cell_plate.subsample_size, gene_plate.size, kinetics_plate.size)

            kinetics_lineage = pyro.sample("kinetics_lineage", Categorical(kinetics_logits),
                                           #infer={'enumerate': 'parallel'})
                                           infer={'enumerate': 'sequential'})

            assert kinetics_lineage.shape == (cell_plate.subsample_size, gene_plate.size), kinetics_lineage.shape
            alpha = Vindex(alpha_k.transpose(kinetics_plate.dim, gene_plate.dim))[..., kinetics_lineage]
            beta = Vindex(beta_k.transpose(kinetics_plate.dim, gene_plate.dim))[..., kinetics_lineage]
            gamma = Vindex(gamma_k.transpose(kinetics_plate.dim, gene_plate.dim))[..., kinetics_lineage]
            switching = Vindex(switching_k.transpose(kinetics_plate.dim, gene_plate.dim))[..., kinetics_lineage]
...
class MultiKineticsModelDirichletLinear(VelocityModel):
    ...
    def forward(
        ...
        with kinetics_plate:   # kinetics specific weights
            kinetics_weights = pyro.sample("kinetics_weights", Beta(self.one*2, self.one*5))
            assert kinetics_weights.shape == (kinetics_plate.size, 1, 1), kinetics_weights.shape
        ...
        with cell_plate:
            ...
            with gene_plate:
                with kinetics_plate:
                    ...
                    ut = torch.einsum("...ijk,ijk->...jk", ut_k, kinetics_weights).unsqueeze(-3)
                    st = torch.einsum("...ijk,ijk->...jk", st_k, kinetics_weights).unsqueeze(-3)
...
class AuxTrajectoryModel(PyroModule):
    ...
    def forward(self, u_obs, s_obs, u_log_library, s_log_library, ind_x):
        ...
        rate = 1. / aux_cell_plate.size  # TODO make time dependent
        u_aux = []
        s_aux = []
        dt_aux = []
        for step in pyro.markov(range(aux_cell_plate.size)):
            ...
            with gene_plate:
                u_aux_noise = pyro.sample(f"u_aux_noise_{step}", Normal(self.zero, self.one))
                s_aux_noise = pyro.sample(f"s_aux_noise_{step}", Normal(self.zero, self.one))
                state_aux = pyro.sample(f"cell_gene_state_{step}", Bernoulli(logits=dt_aux[step] - switching)) == self.zero
        ...
        with aux_cell_plate, gene_plate:
            u_aux_obs = pyro.sample("u_aux_obs", Normal(u_aux, u_scale), obs=self.aux_u_obs_init)
            s_aux_obs = pyro.sample("s_aux_obs", Normal(s_aux, s_scale), obs=self.aux_s_obs_init)

        # approximation: real cells independent
        prob_real_to_aux = (torch.ones(self.num_aux_cells) / self.num_aux_cells).to(u_obs.device)
        with cell_plate:
            order = pyro.sample("order", Categorical(prob_real_to_aux), infer={"enumerate": "parallel"})
            dt = dt_aux[..., order]
            with gene_plate:
                ...
                state = pyro.sample("cell_gene_state", Bernoulli(logits=dt - switching)) == self.zero
                alpha = torch.where(state, alpha, self.zero)
                ...
                u_obs = pyro.sample("u_obs", Normal(u * u_scale / s_scale, u_scale), obs=u_obs)
...
class DecoderTimeModel(LogNormalModel):
    ...
    def forward(
        ...
        with cell_plate:
            z_loc = u_obs.new_zeros(torch.Size((u_obs.shape[0], 1, self.latent_dim)))
            z_scale = u_obs.new_ones(torch.Size((u_obs.shape[0], 1, self.latent_dim)))
            z = pyro.sample("latent_time_latent_space", dist.Normal(z_loc, z_scale).to_event(1)) # (cells, 1, components)

        z = z.squeeze()
        t_scale, _, t_rate, t_dropout = self.decoder("gene", z, s_log_library)

        with cell_plate, gene_plate:
            state = pyro.sample("cell_gene_state", Bernoulli(logits=t_scale-switching)) == self.zero
            u0_vec = torch.where(state, self.zero, u_inf)
            s0_vec = torch.where(state, self.zero, s_inf)
            alpha_vec = torch.where(state, alpha, self.zero)
            ...
            ut, st = mRNA(tau, u0_vec, s0_vec, alpha_vec, beta, gamma)
            u_dist, s_dist = self.get_likelihood(ut, st, u_log_library, s_log_library)
            u = pyro.sample("u", u_dist, obs=u_obs)
            s = pyro.sample("s", s_dist, obs=s_obs)
...
class LatentFactor(LogNormalModel):
    ...
    def model(
        ...
        cell_plate, gene_plate = self.create_plates(u_obs, s_obs, u_log_library, s_log_library, ind_x)
        cell_codebook_scale = pyro.param("cell_codebook_scale", lambda: torch.tensor(0.1), constraint=positive).to(u_obs.device)
        cell_codebook = pyro.sample("cell_codebook", Normal(self.zero, cell_codebook_scale).
                                    expand((self.latent_factor_size, self.num_genes * 2)).to_event(2).mask(False))
        cell_code_scale = pyro.param("cell_code_scale", lambda: torch.tensor(0.1), constraint=positive).to(u_obs.device)
        with cell_plate, poutine.mask(mask=False):
            cell_code = pyro.sample("cell_code", Normal(self.zero, cell_code_scale).expand((self.latent_factor_size,)).to_event(1))
    ...
    def model2(
        ...
        cell_codebook_scale = pyro.param("cell_codebook_scale", lambda: torch.tensor(0.1), constraint=positive).to(u_obs.device)
        cell_code_scale = pyro.param("cell_code_scale", lambda: torch.tensor(0.1), constraint=positive).to(u_obs.device)
        cell_codebook = pyro.sample("cell_codebook", Normal(self.zero, cell_codebook_scale).expand((self.latent_factor_size, self.num_genes * 2)).to_event(2))

        u_scale = pyro.sample("u_scale", Normal(self.zero, self.one).expand((self.num_genes, )).to_event(1))
        s_scale = pyro.sample("s_scale", Normal(self.zero, self.one).expand((self.num_genes, )).to_event(1))

        u_pcs_mean = pyro.sample("u_pcs_mean", Normal(self.zero, self.one).expand((self.num_genes, )).to_event(1))
        s_pcs_mean = pyro.sample("s_pcs_mean", Normal(self.zero, self.one).expand((self.num_genes, )).to_event(1))

        with cell_plate:
            cell_code = pyro.sample("cell_code", Normal(self.zero, cell_code_scale).expand((self.latent_factor_size,)).to_event(1))
            regressor_output = torch.einsum("ab,bd->ad", cell_code, cell_codebook.squeeze())
            regressor_u = softplus(regressor_output[..., :self.num_genes] + u_pcs_mean)
            regressor_s = softplus(regressor_output[..., self.num_genes:] + s_pcs_mean)
            u_dist, s_dist = self.get_likelihood(regressor_u, regressor_s, u_log_library, s_log_library, None, None)
            u = pyro.sample("u", u_dist.to_event(1), obs=u_obs)
            s = pyro.sample("s", s_dist.to_event(1), obs=s_obs)
    ...
    def get_rna(self, u_scale, s_scale,
        ...
        if self.cell_specific_kinetics is None:
            if self.guide_type == 'auto':
                ...
                if pyro.__version__.startswith('1.8.1'): # parallel still memory leaky from pip install
                    enum = 'parallel'
            ...
            state = pyro.sample("cell_gene_state", Bernoulli(logits=t-switching),
                                infer={'enumerate': enum}) == self.zero
            alpha_off = self.zero
            u0_vec = torch.where(state, u0, u_inf)
            s0_vec = torch.where(state, s0, s_inf)
            alpha_vec = torch.where(state, alpha, alpha_off)
