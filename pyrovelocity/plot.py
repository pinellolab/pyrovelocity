...
import pyro
from .utils import mRNA, mse_loss_sum
...
def plot_dynamic_pyro(
    ...
    alpha = torch.tensor(alpha) if fix_param_list[0] == 1 else pyro.param("AutoDelta.alpha_sample")
    beta = torch.tensor(beta) if fix_param_list[1] == 1 else pyro.param("AutoDelta.beta_sample")
    gamma = torch.tensor(gamma) if fix_param_list[2] == 1 else pyro.param("AutoDelta.gamma_sample")
    scale = torch.tensor(scale) if fix_param_list[3] == 1 else pyro.param("AutoDelta.scale_sample")
    t_ = torch.tensor(t_) if fix_param_list[4] == 1 else pyro.param("AutoDelta.switching_sample")
...
def plot_multigenes_dynamical(summary, alpha, beta, gamma, t_, t, adata, gene="Cpe", scale=None, ax=None, raw=False):
    import numpy as np
    ...
    if scale is not None:
        ynew = (gamma / beta * (xnew - torch.min(xnew))) * scale + torch.min(ut * scale + u0)
    else:
        ynew = gamma / beta * (xnew - torch.min(xnew)) + torch.min(ut)
    ...
    try:
        if raw:
            scv.pl.scatter(adata, gene, x='spliced', y='unspliced', color=["clusters"], ax=ax, show=False)
        else:
            scv.pl.scatter(adata, gene, color=["clusters"], ax=ax, show=False)
    except:
        if raw:
            ...
            scv.pl.scatter(adata, gene, color=["Clusters"], ax=ax, show=False)
    ax.plot(
        st.detach().numpy(), ut.detach().numpy(), linestyle="-", linewidth=2.5, color="red",
        label='Pyro-Velocity'
    )
    ax.plot(
        xnew.detach().numpy(),
    ...
    if summary is not None:
        ax.scatter(summary['x_obs']['mean'][:, 1],
                   summary['x_obs']['mean'][:, 0], alpha=0.5,
                   color='red')


def plot_posterior_time(pos, adata, ax=None, fig=None, basis='umap', addition=True,
                        position='left', s=3):
    if addition:
        sns.set_style('white')
    ...
    im = ax.scatter(adata.obsm[f'X_{basis}'][:, 0], adata.obsm[f'X_{basis}'][:, 1], s=s, alpha=0.4,
                    c=adata.obs['cell_time'], cmap='inferno',
                    linewidth=0)
    ...
    ax.set_title("Pyro-Velocity shared time\ncorrelation: %.2f" % (spearmanr(adata.obs['cell_time'].values, 1-adata.obs.cytotrace.values)[0]), fontsize=7)
    #fig.tight_layout()
...
def plot_gene_ranking(pos, adata, ax=None, time_correlation_with='s', selected_genes=None, assemble=False, data='correlation',
                      negative=False, adjust_text=False):
    ...
    for p, ad, label in zip(pos, adata, ['train', 'valid']):
        for sample in range(30):
            maes_list.append(mae_per_gene(p['s'][sample].squeeze(), ad.layers['raw_spliced'].toarray()))
            df_genes_cors = compute_similarity2(p[time_correlation_with][sample].squeeze(), p['cell_time'][sample].squeeze().reshape(-1, 1))
    ...
    volcano_data.loc[:, 'time_correlation_rank'] = volcano_data.time_correlation.apply(abs).rank(ascending=False)
    volcano_data.loc[:, 'rank_product']          = volcano_data.mean_mae_rank * volcano_data.time_correlation_rank
    ...
    if selected_genes is None:
        ...
        genes = volcano_data.sort_values("mean_mae", ascending=False).head(300).sort_values("time_correlation", ascending=negative).head(4).index
    else:
    ...
    if data == 'correlation':
        sns.scatterplot(x='time_correlation', y='mean_mae', hue='selected genes', data=volcano_data, s=3, linewidth=0, ax=ax, legend=False, alpha=0.3)
        ...
        for i, g in enumerate(genes):
            ax.scatter(volcano_data.loc[g, :].time_correlation, volcano_data.loc[g, :].mean_mae,
                       s=15, color='red', marker='*')
            #texts.append(ax.text(volcano_data.loc[g, :].time_correlation-0.45, volcano_data.loc[g, :].mean_mae-0.22*i,
            texts.append(ax.text(volcano_data.loc[g, :].time_correlation, volcano_data.loc[g, :].mean_mae, g, fontsize=5, color='black', ha='center', va='center'))
            if not assemble:
                if i%2==0:
                    offset = 10+i*5
                else:
                ...
                if not adjust_text:
                    ...
                    adjust_text(texts,
                                ...
                                force_text=(0.01, 0.25), force_points=(0.01, 0.25),                                        arrowprops=dict(arrowstyle='-', color='blue', alpha=.6),
                                ax=ax)
    else:
        sns.scatterplot(x='switching', y='mean_mae', hue='selected genes', data=volcano_data, s=3, linewidth=0, ax=ax, legend=False, alpha=0.3)
        ax.set_title("Pyro-Velocity genes", fontsize=7)
...
def denoised_umap(pos, adata, cell_state='state_info'):
    ...
    projection = [
        ('PCA', sklearn.decomposition.PCA(random_state=99,
                                          n_components=50)),
    ...
    if 'u_scale' in pos:
        adata.layers['velocity_pyro'] = (pos['ut'] * pos['beta']/ (pos['u_scale']/pos['s_scale']) - pos['st'] * pos['gamma']).mean(0)
    else:
        adata.layers['velocity_pyro'] = (pos['ut'] * pos['beta'] - pos['st'] * pos['gamma']).mean(0)
    ...
    scv.pl.velocity_embedding_grid(adata, basis='umap1', vkey='velocity_pyro',
                                   density=0.5, scale=0.25, arrow_size=3, color=cell_state,
    ...
    scv.tl.velocity_graph(adata, vkey='velocity_pyro', xkey='spliced_pyro')
    scv.tl.velocity_embedding(adata, vkey='velocity_pyro', basis='umap1')
    scv.pl.velocity_embedding_grid(adata, basis='umap2', vkey='velocity_pyro', color=cell_state,
                                   density=0.5, scale=0.25, arrow_size=3, show=False,
                                   ax=ax[1][1])
...
def vector_field_uncertainty(adata, pos, basis='tsne', n_jobs=1, denoised=False):
    ...
    if 'beta_k' in pos:
        velocity_samples = pos['ut'] * pos['beta_k'] / scale - pos['st'] * pos['gamma_k']
    else:
    ...
    for sample in range(30):
        ...
        scv.tl.velocity_graph(adata, vkey='velocity_pyro', xkey='spliced_pyro', n_jobs=n_jobs)
...
def plot_vector_field_uncertain(adata, embed_mean, embeds_radian, fig=None, cbar=True, basis='umap', scale=0.002,
                                cbar_pos=[0.22, 0.28, 0.5, 0.05], p_mass_min=3.5, only_grid=False, ax=None,
                                autoscale=False,
                                density=0.3, arrow_size=5):
    from astropy.stats import circstd
    ...
    if (not only_grid):
        ...
        #colormap = cm.inferno
        import seaborn as sns
        adata.obs['uncertain'] = get_posterior_sample_angle_uncertainty(embeds_radian/np.pi*180)
        im = ax[0].scatter(adata.obsm[f'X_{basis}'][:, 0],
    ...
    X_grid, V_grid, uncertain = project_grid_points(adata.obsm[f'X_{basis}'], embed_mean, get_posterior_sample_angle_uncertainty(embeds_radian/np.pi*180), p_mass_min=p_mass_min, autoscale=autoscale, density=density)
    #print(X_grid)
    #print(V_grid)
    #print(uncertain)
    ...
    colormap = cm.winter
    ax.scatter(adata.obsm[f'X_{basis}'][:, 0], adata.obsm[f'X_{basis}'][:, 1], s=1, linewidth=0, color='gray', alpha=0.2)
    im=ax.quiver(X_grid[:, 0],
                 X_grid[:, 1],
                 V_grid[:, 0],
                 V_grid[:, 1],
                 #uncertain,
...
def plot_mean_vector_field(pos, adata, ax, basis='umap', n_jobs=1, scale=0.2, density=0.4,
                           spliced='spliced_pyro', raw=False):
    import umap
    import sklearn
    import copy
    from sklearn.pipeline import Pipeline
    ...
    if spliced == 'spliced_pyro':
        ...
        if ('u_scale' in pos): # only one scale for Poisson distribution
            adata.layers['velocity_pyro'] = (ut * pos['beta']/pos['u_scale'] - st * pos['gamma']).mean(0)
        else:
            if 'beta_k' in pos:
                adata.layers['velocity_pyro'] = (ut * pos['beta_k'] - pos['st'] * pos['gamma_k']).mean(0).squeeze()
            else:
                adata.layers['velocity_pyro'] = (ut * pos['beta'] - pos['st'] * pos['gamma']).mean(0)
        scv.tl.velocity_graph(adata, vkey='velocity_pyro', xkey='spliced_pyro', n_jobs=n_jobs)
    elif spliced in ['Ms']:
        ut = adata.layers['Mu']
        st = adata.layers['Ms']
        if ('u_scale' in pos) and ('s_scale' in pos):
            adata.layers['velocity_pyro'] = (ut * pos['beta']/(pos['u_scale']/pos['s_scale']) - st * pos['gamma']).mean(0)
        else:
            adata.layers['velocity_pyro'] = (ut * pos['beta'] - pos['st'] * pos['gamma']).mean(0)
        ...
        if ('u_scale' in pos) and ('s_scale' in pos):
            ...
            adata.layers['velocity_pyro'] = (ut * pos['beta'] - pos['st'] * pos['gamma']).mean(0)
        scv.tl.velocity_graph(adata, vkey='velocity_pyro', xkey='spliced', n_jobs=n_jobs)

    scv.tl.velocity_embedding(adata, vkey='velocity_pyro', basis=basis)
    scv.pl.velocity_embedding_grid(adata, basis=basis, vkey='velocity_pyro', linewidth=1,
                                   ax=ax, show=False, legend_loc='on data', density=density, scale=scale, arrow_size=3)
...
def project_grid_points(emb, velocity_emb, uncertain=None, p_mass_min=1.0, density=0.3,
                        autoscale=False):
    ...
    if (len(velocity_emb.shape) == 2):
        V_grid = (velocity_emb[:, :2][neighs] * weight[:, :, None]).sum(1) / \
            np.maximum(1, p_mass)[:, None]
    else:
        V_grid = (velocity_emb[:, :2][neighs] * weight[:, :, None, None]).sum(1) / \
    ...
    if uncertain is None:
        ...
        uncertain = (uncertain[neighs] * weight).sum(1) / \
            np.maximum(1, p_mass)
        return X_grid[p_mass>p_mass_min], V_grid[p_mass>p_mass_min], uncertain[p_mass>p_mass_min]


def plot_arrow_examples(adata, v_maps, embeds_radian, ax=None, fig=None, cbar=True, basis='umap',
                        n_sample=30, scale=0.0021, alpha=0.02, index=19, scale2=0.04, num_certain=3, num_total=4, p_mass_min=1.0, density=0.3, arrow_size=4):
    from astropy.stats import circstd
    from scvelo.plotting.velocity_embedding_grid import default_arrow, compute_velocity_on_grid
    X_grid, V_grid, uncertain = project_grid_points(adata.obsm[f'X_{basis}'], v_maps, get_posterior_sample_angle_uncertainty(embeds_radian/np.pi*180), p_mass_min=p_mass_min, density=density)
    norm = Normalize()
    norm.autoscale(uncertain)
    colormap = cm.winter
    ...
    quiver_kwargs.update({"linewidth": 1, "zorder": 3})
    ax.scatter(adata.obsm[f'X_{basis}'][:, 0], adata.obsm[f'X_{basis}'][:, 1],
               s=1, linewidth=0, color='gray', alpha=alpha)
    for i in range(n_sample):
        for j in indexes:
            ax.quiver(X_grid[j, 0],
                      X_grid[j, 1],
                      V_grid[j][0][i],
                      V_grid[j][1][i],
                      ...
                      norm=Normalize(vmin=0, vmax=360),
                      scale=scale,
                      color=colormap(norm(uncertain))[j], **quiver_kwargs)
...
def set_colorbar(smp, ax, orientation="vertical", labelsize=None, fig=None, position='right', rainbow=False):
    from matplotlib.ticker import MaxNLocator
    from mpl_toolkits.axes_grid1.inset_locator import inset_axes
...
def us_rainbowplot(genes, adata, pos, data=['st', 'ut'], cell_state='clusters'):
    ...
    for gene in genes:
        ...
        ress = pd.DataFrame({"cell_time": pos['cell_time'].mean(0).squeeze(),
                             "cell_type": adata.obs[cell_state].values,
                             "spliced": pos_s[:, index].squeeze(),
                             "unspliced": pos_u[:, index].squeeze()})
        if n == 2:
            sns.scatterplot(x='cell_time', y='spliced', data=ress, alpha=0.4,
                            linewidth=0, edgecolor="none",hue='cell_type', ax=ax1, marker='o', legend='brief',
                            palette='bright', s=10)
        else:
            sns.scatterplot(x='cell_time', y='spliced', data=ress, alpha=0.4,
                            linewidth=0, edgecolor="none",
                            palette='bright', hue='cell_type', ax=ax1, marker='o', legend='brief', s=10)
        ...
        ax2.set_ylabel("")  
        ax2.set_xlabel("")
        sns.scatterplot(x='spliced', y='unspliced', data=ress, alpha=0.4, s=25,
                        edgecolor="none", hue='cell_type', ax=ax2, legend=False, marker="*",
                        palette='bright')
        if n == 3:
...
def rainbowplot(volcano_data, adata, pos, fig=None, genes=None, data=['st', 'ut'], cell_state='clusters', basis='umap', num_genes=5, add_line=True, negative=False,
                scvelo_colors=False):
    import matplotlib.lines as mlines
    import matplotlib.pyplot as plt
    if genes is None:
        ...
        genes = volcano_data.sort_values("mean_mae", ascending=False).head(300).sort_values("time_correlation", ascending=negative).head(num_genes).index
    ...
    if scvelo_colors:
        scv.pl.scatter(adata, basis=basis, fontsize=7, 
               legend_loc='on data', legend_fontsize=7, 
               color=cell_state, show=False)
        colors = dict(zip(adata.obs.state_info.cat.categories, adata.uns['state_info_colors']))
    else:
        clusters = adata.obs.loc[:, cell_state]
        colors = dict(zip(clusters.cat.categories, sns.color_palette("deep", clusters.cat.categories.shape[0])))
    ...
    for gene in genes:
        ...
        print(pos_mean_time)
        ress = pd.DataFrame({"cell_time": pos_mean_time/pos_mean_time.max(),
                             "cell_type": adata.obs[cell_state].values,
        ...
        if add_line:
            ...
            for row in range(ress.shape[0]):
                ...
                ax1.vlines(x=ress.cell_time[row], ymin=0, ymax=ress.spliced[row], colors=colors[ress.cell_type[row]], alpha=0.1)
...
#         ax2.set_ylabel("")
        ...
        #ress = pd.DataFrame({"cell_time": pos['cell_time'].mean(0).flatten(),
        ress = pd.DataFrame({"cell_type": adata.obs[cell_state].values,
                             "unspliced": ut[:, index].flatten(),
                             "spliced": st[:, index].flatten()})
        #im = ax2.scatter(st[:, index], ut[:, index], c=ress.cell_time, s=25, cmap = 'seismic')   
        sns.scatterplot(x='spliced', y='unspliced', data=ress, alpha=0.4,
                        linewidth=0, edgecolor="none",hue='cell_type',
                        #palette=dict(zip(adata.obs.clusters.cat.categories, adata.uns['clusters_colors'])),
                        palette=colors,
                        ax=ax2, marker='o', legend=False, s=3)
        ax2.set_xlabel("")
...
#scv.pl.umap(adata, color=gene, ax=ax3, show=False, fontsize=9, title='') #, legend_fontsize=7)
        im = ax3.scatter(adata.obsm[f'X_{basis}'][:, 0], adata.obsm[f'X_{basis}'][:, 1], s=3,
                         c=st[:, index].flatten(), cmap='RdBu_r')
        ...
        n += 1
    sns.despine()
    subfigs[0].subplots_adjust(hspace=0.8, wspace=1.4, left=0.26, right=0.94, top=0.92, bottom=0.12)
    subfigs[1].subplots_adjust(hspace=0.8, wspace=0.4, left=0.2, right=0.7, top=0.92, bottom=0.08)
    # subfigs[0].text(0.073, 0.58, "unspliced expression", size=7, rotation="vertical", va="center")
    subfigs[0].text(-0.01, 0.58, "unspliced expression", size=7, rotation="vertical", va="center")
    subfigs[0].text(0, 0.97, "f",
                    fontsize=7, fontweight='bold', va='top', ha='right')
    subfigs[0].text(0.552, 0.58, "spliced expression", size=7, rotation="vertical", va="center")
    return fig


def plot_state_uncertainty(pos, adata, kde=True, data='denoised', top_percentile=0.9, ax=None,
                           basis='umap'):
    if data == 'denoised':
        adata.obs['state_uncertain'] = \
          np.sqrt((((pos['st'] - pos['st'].mean(0))**2+((pos['ut'] - pos['ut'].mean(0)))**2).sum(-1))).mean(0)
    else:
        adata.obs['state_uncertain'] = \
          np.sqrt((((pos['s'] - pos['s'].mean(0))**2+((pos['u'] - pos['u'].mean(0)))**2).sum(-1))).mean(0)

    ax = scv.pl.scatter(adata, basis=basis, color='state_uncertain', cmap='RdBu_r', ax=ax, show=False, colorbar=True, fontsize=7)

    if kde:
        select = adata.obs['state_uncertain'] > np.quantile(adata.obs['state_uncertain'], top_percentile)
        sns.kdeplot(adata.obsm[f'X_{basis}'][:, 0][select],
                    adata.obsm[f'X_{basis}'][:, 1][select], ax=ax, levels=3, fill=False)
    else:
        select = None
    return select, ax
...
def align_trajectory_diff(adatas,
                          velocity_embeds,
                          density=0.3, smooth=.5,
                          input_grid=None,
                          input_scale=None, min_mass=1.0,
                          embed='umap', autoscale=False,
                          length_cutoff=10):
    from sklearn.neighbors import NearestNeighbors
    from scipy.stats import norm as normal
    ...
    for adata, velocity_embed in zip(adatas, velocity_embeds):
        ...
        p_mass = weight.sum(1)
        V_grid = (velocity_embed[neighs] * weight[:, :, None]).sum(1) / \
            np.maximum(1, p_mass)[:, None]
        if autoscale:
            V_grid /= 3 * quiver_autoscale(X_grid, V_grid)
    ...
    if input_grid is None and input_scale is None:
        ...
        mass_index = reduce(np.intersect1d, [np.where(p_mass > min_mass)[0] for p_mass in p_mass_list])
