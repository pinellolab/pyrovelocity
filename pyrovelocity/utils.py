...
def ode_mRNA(tau, u0, s0, alpha, beta, gamma):
    ...
    dx_dt = rescale_time(dx_dt, torch.tensor(0., dtype=tau.dtype, device=tau.device), tau)
    grid = torch.linspace(0., 1., 100, dtype=tau.dtype, device=tau.device)
...
def mRNA(tau, u0, s0, alpha, beta, gamma):
    ...
    st = s0 * exps + alpha / gamma * (1 - exps) + expus  # remove expus is the most stable, does it theoretically make sense?
...
def site_is_discrete(site: dict) -> bool:
    return site["type"] == "sample" and not site["is_observed"] and getattr(site["fn"], "has_enumerate_support", False)


def init_with_all_cells(adata, input_type='knn', shared_time=True, latent_factor='linear',
                        latent_factor_size=10, plate_size=2,
                        num_aux_cells=200, init_smooth=True):
    ...
    if 'Mu' in adata.layers and 'Ms' in adata.layers and init_smooth:
        ...
        u_obs = torch.tensor(adata.layers['unspliced'].toarray() if issparse(adata.layers['unspliced']) else adata.layers['unspliced'], dtype=torch.float32)
        s_obs = torch.tensor(adata.layers['spliced'].toarray() if issparse(adata.layers['spliced']) else adata.layers['spliced'], dtype=torch.float32)
    else:
        raise

    ub_u = torch.stack([torch.quantile(u[u > 0], 0.99) for u in torch.unbind(u_obs, axis=1)])
    ub_s = torch.stack([torch.quantile(s[s > 0], 0.99) for s in torch.unbind(s_obs, axis=1)])
    s_mask = (s_obs > 0) & (s_obs <= ub_s)
    u_mask = (u_obs > 0) & (u_obs <= ub_u)
    #include zeros
    training_mask = s_mask & u_mask

    u_scale = torch.stack([u[u>0].std() for u in torch.unbind(u_obs * training_mask, axis=1)])
    s_scale = torch.stack([s[s>0].std() for s in torch.unbind(s_obs * training_mask, axis=1)])
    scale = u_scale / s_scale

    lb_steady_u = torch.stack([torch.quantile(u[u > 0], 0.98) for u in torch.unbind(u_obs * training_mask, axis=1)])
    lb_steady_s = torch.stack([torch.quantile(s[s > 0], 0.98) for s in torch.unbind(s_obs * training_mask, axis=1)])

    steady_u_mask = training_mask & (u_obs >= lb_steady_u)
    steady_s_mask = training_mask & (s_obs >= lb_steady_s)

    u_obs = u_obs / scale
    u_inf = (u_obs * (steady_u_mask|steady_s_mask)).sum(dim=0)/(steady_u_mask|steady_s_mask).sum(dim=0)
    s_inf = (s_obs * steady_s_mask).sum(dim=0) / steady_s_mask.sum(dim=0)

    gamma = (u_obs * steady_s_mask * s_obs).sum(axis=0) / ((steady_s_mask * s_obs) ** 2).sum(axis=0)+1e-6
    gamma = torch.where(gamma < 0.05 / scale, gamma * 1.2, gamma)
    gamma = torch.where(gamma > 1.5 / scale, gamma / 1.2, gamma)
    alpha = gamma * s_inf
    beta  = alpha / u_inf

    switching = tau_inv(u_inf, s_inf, 0., 0., alpha, beta, gamma)
    tau = tau_inv(u_obs, s_obs, 0., 0., alpha, beta, gamma)
    tau = torch.where(tau >= switching, switching, tau)
    tau_ = tau_inv(u_obs, s_obs, u_inf, s_inf, 0., beta, gamma)
    tau_ = torch.where(tau_ >= tau_[s_obs > 0].max(dim=0)[0], tau_[s_obs > 0].max(dim=0)[0], tau_)
    ut, st = mRNA(tau, 0., 0., alpha, beta, gamma)
    ut_, st_ = mRNA(tau_, u_inf, s_inf, 0., beta, gamma)
    ...
    init_values = {'alpha': alpha, 'beta': beta, 'gamma': gamma,
                   'switching': switching, 'latent_time': t,
                   'u_scale': u_scale, 's_scale': s_scale,
                   'u_inf': u_inf, 's_inf': s_inf}
    if input_type == 'knn':
    ...
    if latent_factor == 'linear':
        ...
        if plate_size == 2:
            init_values['cell_code'] = torch.tensor(X_train_pca).unsqueeze(-1).transpose(-1, -2)
        else:
    ...
    if num_aux_cells > 0:
        ...
        if 'cytotrace' in adata.obs.columns:
            ...
            order_aux_list = []
            for i in order_aux:
                order_aux_list.append(np.random.choice(np.where((adata.obs['cytotrace'].values>i[0]) & (adata.obs['cytotrace'].values < i[-1]))[0]))
            order_aux = np.array(order_aux_list)
        else:
            order_aux = np.argsort((adata.layers['spliced'].toarray() > 0).sum(axis=1))[::-1]
        ...
        if latent_factor == 'linear':
            init_values['cell_code_aux'] = torch.tensor(X_train_pca).unsqueeze(-1).transpose(-1, -2)[:num_aux_cells]
    ...
    if shared_time:
        ...
        if num_aux_cells > 0:
            ...
            init_values['latent_time_aux'] = init_values['latent_time_aux'] - init_values['cell_time_aux']
...
def get_velocity_samples(posterior_samples, model):
    ...
    scale = (posterior_samples['u_scale'][:, 0, :]/posterior_samples['s_scale'][:, 0, :]).mean(0)
...
def mae_evaluate(pos, adata):
    ...
    if isinstance(pos, tuple):
        for model_label, model_obj, split_index in zip(['Poisson train',
                                                        'Poisson valid'],
                                          pos[:2],
                                          pos[2:]):
            for sample in range(30):
                maes_list.append(mae(np.hstack([model_obj['u'][sample], model_obj['s'][sample]]),
                    np.hstack([adata.layers['raw_unspliced'].toarray()[split_index],
        ...
        for model_label, model_obj in zip(['Poisson all cells'], [pos]):
            for sample in range(30):
                maes_list.append(mae(np.hstack([model_obj['u'][sample], model_obj['s'][sample]]),
                    np.hstack([adata.layers['raw_unspliced'].toarray(),
