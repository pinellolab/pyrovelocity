import pyro.contrib.easyguide as easyguide
from scvi.nn import DecoderSCVI, Encoder, FCLayers, Decoder
...
from pyro.distributions.constraints import positive, greater_than
from pyro.distributions import TransformedDistribution, \
    LowRankMultivariateNormal, Delta, Bernoulli, transforms, Normal, LogNormal, Categorical
from pyro.distributions.transforms import (
    ExpTransform, SoftplusTransform
)
from ._velocity_model import VelocityModel, LatentFactor
from .utils import tau_inv, mRNA, debug, site_is_discrete
...
class VelocityGuide(easyguide.EasyGuide):
    def __init__(self,
        ...
        if hasattr(self, 'alpha_init') and hasattr(self, 'u_inf_init'): # with initialization of steady-state
            self.u_inf = PyroParam(self.u_inf_init.clone(), constraint=positive, event_dim=self.event_dim)
            self.s_inf = PyroParam(self.s_inf_init.clone(), constraint=positive, event_dim=self.event_dim)
        else:
            self.u_inf = PyroParam(torch.ones(self.num_genes), constraint=positive, event_dim=0)
            self.s_inf = PyroParam(torch.ones(self.num_genes), constraint=positive, event_dim=0)
        ...
        if self.likelihood != 'Normal':
            hidden_size = 128
            self.neuralnetwork = nn.Sequential(nn.Linear(self.num_genes*2, hidden_size),
                                               nn.ReLU(),
        ...
        if hasattr(self, 'alpha_init') and hasattr(self, 'u_inf_init'):
            if self.num_aux_cells > 0:
                if site['name'] == 'aux/cell_code':
                    return autoguide.init_to_value(site, values={site["name"]: self.cell_code_aux_init})
                if site['name'] == 'aux/latent_time':
                    return autoguide.init_to_value(site, values={site["name"]: pyro.subsample(self.latent_time_aux_init, event_dim=0)})
                if site['name'] == 'aux/cell_gene_state':
                    return autoguide.init_to_value(site, values={site["name"]: pyro.subsample(self.cell_gene_state_aux_init, event_dim=0)})
                if site['name'] == 'aux/cell_time':
                    return autoguide.init_to_value(site, values={site["name"]: pyro.subsample(self.cell_time_aux_init, event_dim=0)})
            if site['name'] == 'cell_code':
                return autoguide.init_to_value(site, values={site["name"]: pyro.subsample(self.cell_code_init, event_dim=1)})
            if site['name'] == 'cell_codebook':
                return autoguide.init_to_value(site, values={site["name"]: pyro.subsample(self.cell_codebook_init, event_dim=1)})
            if site['name']  == 'latent_time':
                if self.num_aux_cells > 0:
                    return autoguide.init_to_value(site, values={site["name"]: pyro.subsample(torch.zeros_like(self.latent_time_init), event_dim=self.event_dim)})
                else:
                    return autoguide.init_to_value(site, values={site["name"]: pyro.subsample(self.latent_time_init, event_dim=self.event_dim)})
            if site['name']  == 'cell_gene_state':
                return autoguide.init_to_value(site, values={site["name"]: pyro.subsample(self.cell_gene_state_init, event_dim=self.event_dim)})
            if site['name'] == 'cell_time':
                assert getattr(self, f"{site['name']}_init").shape[-1] == 1
                print('guide init-------')
                print(self.cell_time_init.shape)
                print(pyro.subsample(self.cell_time_init, event_dim=self.event_dim).shape)
            ...
            if site['name'] == 't_scale':
                ...
                return autoguide.init_to_value(site, values={site["name"]: t_scale_init})
            ...
            if hasattr(self, f"{site['name']}_init"):
                return autoguide.init_to_value(site, values={site["name"]: getattr(self, f"{site['name']}_init")})
    ...
    def guide(
        ...
        if self.plate_size == 2:
            ...
            if self.latent_factor == 'linear':
                ...
                with cell_plate:
                    cell_code_loc = torch.cat((u_obs - u_pcs_mean, s_obs - s_pcs_mean), dim=-1) @ encoder_weights
                    cell_code_loc = cell_code_loc.unsqueeze(-1).transpose(-1, -2)
                    cell_code = pyro.sample("cell_code", Normal(cell_code_loc, cell_code_scale).to_event(1))

            with cell_plate, gene_plate, \
                    poutine.mask(mask=pyro.subsample(self.mask.to(alpha.device), event_dim=0)):
                if self.latent_factor == 'linear':
                    regressor_output = torch.einsum("abc,cd->ad", cell_code, decoder_weights.squeeze())
                ...
                if self.latent_factor_operation == 'sum':
                    u_obs = (u_obs - regressor_u).clamp(0.)
                    s_obs = (s_obs - regressor_s).clamp(0.)
                ...
                state = pyro.sample("cell_gene_state", Bernoulli(logits=cell_gene_state_logits).to_event(self.event_dim)) == self.zero
                ...
                if self.latent_factor_operation == 'selection':
                    ...
                    velocity_model = ((ut - u_)/std_u) ** 2 + ((st - s_)/s_scale) ** 2
                    pca_model = ((regressor_u/scale - u_)/std_u) ** 2 + ((regressor_s - s_)/s_scale) ** 2
                    p_gene_type = (pca_model - velocity_model) #.sum(axis=-2)
                    gene_type = pyro.sample("genecellpair_type", Bernoulli(logits=p_gene_type))
        else:
    ...
    def guide2(
        ...
        s_scale = self.map_estimate("s_scale")
        scale = u_scale / s_scale
        u_inf = pyro.param("u_inf", lambda: self.u_inf_init.clone(),
                           constraint=positive, event_dim=self.event_dim).to(u_obs.device)
        s_inf = pyro.param("s_inf", lambda: self.s_inf_init.clone(),
                           constraint=positive, event_dim=self.event_dim).to(u_obs.device)
        switching = tau_inv(u_inf, s_inf, self.zero, self.zero, alpha, beta, gamma)
...
class AuxCellVelocityGuide(VelocityGuide):
    def __init__(self, *args, **kwargs):
        ...
        self.infer_cell = CellGuide(self.num_cells, self.num_genes, self.cell_time_init, self.zero, self.one, self.shared_time, self.likelihood, self.latent_factor, self.latent_factor_operation, self.t_scale_on)
        if self.num_aux_cells > 0:
            self.infer_aux_cell = CellGuide(self.num_aux_cells, self.num_genes, self.cell_time_aux_init, self.zero, self.one, self.shared_time, self.likelihood, self.latent_factor, self.latent_factor_operation, self.t_scale_on)
    ...
    def guide(
        ...
        with gene_plate:
            ...
            if self.only_cell_times:
                ...
                u_inf, s_inf = mRNA(dt_switching, u0, s0, self.alpha, self.beta, self.gamma)
                ...
                u_inf = pyro.param("u_inf", lambda: self.u_inf_init.clone(), constraint=positive, event_dim=self.event_dim).to(u_obs.device)
                s_inf = pyro.param("s_inf", lambda: self.s_inf_init.clone(), constraint=positive, event_dim=self.event_dim).to(u_obs.device)
                switching = tau_inv(u_inf, s_inf, self.zero, self.zero, alpha, beta, gamma)
                pyro.sample("switching", Delta(switching))
        ...
        with cell_plate: #, poutine.mask(mask=cell_gene_mask):
            u_read_depth = None 
            s_read_depth = None 
            ut, st, u_, s_, regressor_u, regressor_s = self.infer_cell(gene_plate, u_obs, s_obs, u_log_library, s_log_library,
                                             alpha, beta, gamma,
                                             #u_inf, s_inf, switching, u_scale, s_scale,
                                             u_inf, s_inf, dt_switching, u_scale, s_scale,
                                             u_pcs_mean, s_pcs_mean, t_scale, decoder_weights, gene_offset, p_velocity, self.only_cell_times,
                                             u_read_depth=u_read_depth,
                                             s_read_depth=s_read_depth,
                                             u0=u0, s0=s0)  # TODO arguments and return type
        if self.num_aux_cells > 0:
            # new: infer latent variables for auxiliary cells
            with pyro.contrib.autoname.scope(prefix="aux"):
                # TODO set self.num_aux_cells
                aux_cell_plate = self.plate("aux_cell_plate", self.num_aux_cells, dim=cell_plate.dim)
                with aux_cell_plate:
                    ...
                    aux_u_log_library = torch.log(aux_u_obs.sum(axis=-1)+1e-6)  # TODO define
                    aux_s_log_library = torch.log(aux_s_obs.sum(axis=-1)+1e-6)  # TODO define
                    aux_ut, aux_st, aux_u_, aux_s_, aux_regressor_u, aux_regressor_s = self.infer_aux_cell(gene_plate, aux_u_obs, aux_s_obs, aux_u_log_library, aux_s_log_library,
                                                                         alpha, beta, gamma,
                                                                         #u_inf, s_inf, switching, u_scale, s_scale,
                                                                         u_inf, s_inf, dt_switching, u_scale, s_scale,
                                                                         u_pcs_mean, s_pcs_mean, t_scale, decoder_weights, gene_offset, p_velocity, self.only_cell_times,
...
class CellGuide(PyroModule):
    ...
    def __init__(self, num_cells, num_genes, init_cell_time, zero, one, shared_time, likelihood, latent_factor, latent_factor_operation, t_scale_on):
        super().__init__()
        ...
        self.cell_code_scale = PyroParam(torch.tensor(0.1), constraint=positive, event_dim=0)
        self.zero = zero
        self.one = one
        self.u_loc = PyroParam(torch.ones(self.num_cells, self.num_genes), constraint=positive, event_dim=0)
        self.s_loc = PyroParam(torch.ones(self.num_cells, self.num_genes), constraint=positive, event_dim=0)
        self.likelihood = likelihood
    ...
    def forward(self, gene_plate, u_obs, s_obs, u_log_library, s_log_library,
                alpha, beta, gamma,
                #u_inf, s_inf, switching, u_scale, s_scale,
                u_inf, s_inf, dt_switching, u_scale, s_scale,
                u_pcs_mean, s_pcs_mean, t_scale, decoder_weights,
                gene_offset, p_velocity=None, only_cell_times=False,
                u_read_depth=None,
                s_read_depth=None, 
                u0=None,
                s0=None):  # TODO arguments and return type
        ...
        if self.latent_factor == 'linear':
            ...
            cell_code_loc = torch.cat((u_obs - u_pcs_mean, s_obs - s_pcs_mean), dim=-1) @ encoder_weights
            cell_code_loc = cell_code_loc.unsqueeze(-1).transpose(-1, -2)
            cell_code = pyro.sample("cell_code", Normal(cell_code_loc, self.cell_code_scale).to_event(1))
        with gene_plate:
            if self.latent_factor == 'linear':
                regressor_output = torch.einsum("abc,cd->ad", cell_code, decoder_weights.squeeze())
                regressor_u = softplus(regressor_output[..., :self.num_genes].squeeze() + u_pcs_mean)
        ...
        if self.shared_time:
            if not only_cell_times:
                cell_time_loc = torch.mean(
                    ((t - gene_offset) / t_scale) if self.t_scale_on else t, # (t - gene_offset),
                    dim=gene_plate.dim,
                    keepdim=True
                )
...
# t_noise[c, g] = t[c, g] - cell_time[c] * t_scale[g]
            ...
            if self.latent_factor_operation == 'selection':
                ...
                probs = softmax(probs, dim=-1)  # probs.sum(-1, keepdim=True)
                p_gene_type = probs[..., 0]
                cellgene_type = pyro.sample("cellgene_type", Bernoulli(p_gene_type)) ## tend to be constant
            if self.latent_factor_operation == 'selection' and self.latent_factor == 'linear':
...
class VelocityAutoGuideList(AutoGuideList):
    def forward(self, *args, **kwargs):
        ...
        with self.plates['cells']:
            ...
            t = self.model.get_time(u_scale, s_scale,
                                    alpha, beta, gamma,
                                    u_obs, s_obs, u0, s0, t0,
                                    dt_switching, u_inf, s_inf, u_read_depth=u_read_depth, s_read_depth=s_read_depth)
            result['cell_time'] = t
        return result
    ...
    def guide(
        ...
        for step in pyro.markov(range(aux_cell_plate.size)):
            ...
            with gene_plate:
                ...
                state_aux = pyro.sample(f"cell_gene_state_{step}", Bernoulli(logits=dt_aux[step] - switching)) == zero
        ...
        with cell_plate:
            ...
            order_stat = ((u_obs.reshape(u_obs.shape[0], 1, u_obs.shape[1]) - u_aux)**2).sum()+((s_obs.reshape(s_obs.shape[0], 1, s_obs.shape[1]) - s_aux)**2).sum(axis=-1)
            order_stat = order_stat.reshape(u_obs.shape[0], 1, u_aux.shape[0])
            order = pyro.sample("order", Categorical(logits=-order_stat), infer={"enumerate": "parallel"})
            dt = dt_aux[..., order]
            with gene_plate:
                ...
                scale = u_scale / s_scale
                std_u = u_scale / scale
                mse_on = ((u_on-u_obs/scale)/std_u)**2 + ((s_on - s_obs)/s_scale)**2
                mse_off = ((u_off-u_obs/scale)/std_u)**2 + ((s_off - s_obs)/s_scale)**2
                state = pyro.sample("cell_gene_state", Bernoulli(logits=mse_on - mse_off))
...
class DecoderTimeGuide(VelocityGuide):
    ...
    def guide(
        ...
        with cell_plate:
            ...
            pyro.sample("latent_time_latent_space", Normal(z_loc, z_scale).to_event(1))  # (cell, 1, components)
...
class GeneTimeEncoder(nn.Module):
    ...
    def forward(self, u_obs, s_obs, alpha, alpha_off, beta, gamma,
                u0, s0,
                u_inf=None, s_inf=None):
        switching = self.network(torch.stack([u_inf, s_inf,
                                              u0.expand_as(u_inf),
                                              s0.expand_as(s_inf),
                                              alpha, beta, gamma], dim=-1))
        switching = switching[..., 0]
        tau = self.network(torch.stack([u_obs, s_obs,
                                        u0.expand_as(u_obs),
                                        s0.expand_as(s_obs),
                                        alpha.expand_as(u_obs),
                                        beta.expand_as(s_obs),
                                        gamma.expand_as(s_obs)], dim=-1))
        tau = tau[..., 0]

        tau_ = self.network(torch.stack([u_obs, s_obs,
                                         u_inf.expand_as(u_obs), s_inf.expand_as(s_obs),
                                         torch.tensor(alpha_off).expand_as(u_obs),
                                         beta.expand_as(u_obs),
                                         gamma.expand_as(u_obs)], dim=-1))
        tau_ = tau_[..., 0]
    ...
    def __init__(self, model: VelocityModel, use_gpu:int, **initial_values):
        if initial_values != {}:
            init_loc_fn = autoguide.init_to_value(
                    values={key: value.to(f'cuda:{use_gpu}') 
                            if key != 'cell_time' else pyro.subsample(value.to(f'cuda:{use_gpu}'), event_dim=0)
                            for key, value in initial_values.items()})
                #return autoguide.init_to_value(site, values={site["name"]: pyro.subsample(self.cell_time_init, event_dim=self.event_dim)})
        else:
            init_loc_fn = autoguide.init_to_median
        super().__init__(
            poutine.block(model, hide_fn=site_is_discrete), init_loc_fn=init_loc_fn, create_plates=model.create_plates
        )
...
class AutoNormalRNAVelocityGuide(autoguide.AutoNormal):
    def __init__(self, model: VelocityModel, use_gpu:int, **initial_values):
        if initial_values != {}:
            init_loc_fn = autoguide.init_to_value(values={key: value.to(f'cuda:{use_gpu}') for key, value in initial_values.items()})
        else:
            init_loc_fn = autoguide.init_to_median
        super().__init__(
            poutine.block(model, hide_fn=site_is_discrete), init_loc_fn=init_loc_fn, create_plates=model.create_plates
...
class LatentGuide(easyguide.EasyGuide):
    ...
    def guide(
        ...
        if self.plate_size == 2:
            ...
            with cell_plate:
                cell_code_loc = torch.cat((u_obs - u_pcs_mean, s_obs - s_pcs_mean), dim=-1) @ encoder_weights
                if self.inducing_point_size > 0:
                    inducing_points = pyro.param("inducing_points", lambda: torch.randn(self.inducing_point_size, self.latent_factor_size)).to(u_obs.device)
                    inducing_mean = pyro.param("inducing_mean", lambda: torch.randn(self.inducing_point_size, self.latent_factor_size)).to(u_obs.device)

                    kernel = pyro.contrib.gp.kernels.RBF(input_dim=self.latent_factor_size)
                    assert cell_code_loc.shape == (cell_plate.subsample_size, self.latent_factor_size)
                    assert inducing_points.shape == (self.inducing_point_size, self.latent_factor_size)
                    assert inducing_mean.shape == (self.inducing_point_size, self.latent_factor_size)
                    cell_code_loc, cell_code_scale = pyro.contrib.gp.util.conditional(
                        cell_code_loc, inducing_points, kernel, inducing_mean.transpose(-1, -2), full_cov=False)
                    assert cell_code_loc.shape == (self.latent_factor_size, cell_plate.subsample_size)
    ...
    def guide2(
        ...
        cell_code_scale = pyro.param("cell_code_scale", lambda: torch.tensor(0.1), constraint=positive).to(u_obs.device)
        with cell_plate:
            cell_code = torch.cat((u_obs - u_pcs_mean, s_obs - s_pcs_mean), dim=-1) @ encoder_weights
            cell_code = pyro.sample("cell_code", Normal(cell_code, cell_code_scale).to_event(1))
        return cell_code
