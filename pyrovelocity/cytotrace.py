...
__all__ = ['census_normalize', 'remove_zero_mvg', 'compute_similarity2', 'compute_similarity1', 'compute_gcs',
           'convert_to_markov', 'any', 'find_nonzero', 'FNNLSa', 'nu', 'eps', 'regressed', 'diffused',
           'compare_cytotrace', 'cytotrace', 'batch_cytotrace', 'compare_cytotrace_ncores', 'cytotrace_ncore',
           'align_diffrate', 'plot_multiadata', 'cumulative_boxplot', 'run_cytotrace']
...
def census_normalize(mat, count):
    ...
    if issparse(mat):
        ...
        #x.data = 2**x.data - 1
        # gene x cell
        x = mat.multiply(count).multiply(1./mat.sum(axis=0)).tocsr()
        return x.log1p()
    else:
        x = 2**mat - 1
        # gene x cell
...
def remove_zero_mvg(mat):
    ...
    disp = var / mean
    return X_topcell[disp >= (np.sort(disp)[::-1])[:1000][-1], :] # in case less than 1000 genes
...
def compute_similarity1(A):
    ...
    DO = A - (np.einsum("nt->t", A, optimize='optimal') / np.double(n)) # compute O - mean(O)
...
def convert_to_markov(sim):
    ...
    Ds[~zero_rows, :] = \
      (Ds[~zero_rows, :].T / Ds[~zero_rows, :].sum(axis=1)).T # tips
    Ds[:, zero_cols] = 0
    return Ds
...
def regressed(markov, gcs, solver='fnnls'):
    ...
    if solver == 'fnnls':
        ...
        # slow as well
        sol = lsq_linear(sparse.csr_matrix(markov), gcs, bounds=(0, np.inf), lsmr_tol='auto', verbose=1)    # from 1997...
        coef = sol.x
    elif solver == 'lasso': 
...
def compare_cytotrace(adata, layer='all', cell_count=10, condition='age',
                      solver='nnls', is_normalized=False, n_cores=4, top_n_genes=200):
    ...
    if not is_normalized:
        if layer == 'all':
            ...
            try:
                X = adata.layers[layer].toarray() if issparse(adata.layers[layer]) else adata.layers[layer]
            except:
    ...
    for cond in np.unique(adata.obs.loc[:, condition].values[cells_selected]):
        ...
        adata.var.iloc[genes_selected,
                       adata.var.columns.get_loc('cytotrace_corrs_%s' % cond)] = corrs_cond

    adata.obs['gcs'] = None
    adata.obs['cytotrace'] = None
    adata.obs['counts'] = None
    adata.obs.iloc[cells_selected, adata.obs.columns.get_loc('gcs')] = gcs # used for re-rank when integrating multiple-samples
    adata.obs.iloc[cells_selected, adata.obs.columns.get_loc('cytotrace')] = rankdata(gcs) / gcs.shape[0] # cytotrace scores
    adata.obs.iloc[cells_selected, adata.obs.columns.get_loc('counts')] = counts # used for re-rank when integrating multiple-samples


def cytotrace_sparse(adata, layer='raw', cell_count=20, solver='nnls', top_n_features=200, skip_regress=False):
    "optimized version"
    proc_time = time()
    ...
    # gene x cell
    X = X.T
    X = X.multiply(1. / csr_matrix.sum(X, axis=0)) * 1e6
    X = X.tocsr()
    ...
    census_X = X.multiply(feature_count_per_cell).multiply(1. / csr_matrix.sum(X, axis=0)).log1p().tocsr()

    # top variable features, feature x cell
    cell_count_per_feature = (census_X > 0).sum(axis=1).A1
    census_X_topcell = census_X[cell_count_per_feature >= 0.05 * census_X.shape[1], :].tocsr()
    ...
    mvg = census_X_topcell[disp >= (np.sort(disp)[::-1])[:1000][-1], :] # in case less than 1000 features
    ...
    # calculate GC scores
    corrs = compute_similarity2(census_X.T.A, feature_count_per_cell.reshape(-1, 1))[0, :]
    # avoid nan, put all nan to -1
    corrs[np.isnan(corrs)] = -1
    ...
    adata.obs.iloc[cells_selected,
                   adata.obs.columns.get_loc('cytotrace')] = scores

    adata.obs['counts'] = np.nan
    adata.obs.iloc[cells_selected,
                   adata.obs.columns.get_loc('counts')] = gcs
    ...
    adata.var.iloc[features_selected, adata.var.columns.get_loc('cytotrace_corrs')] = np.array(corrs, dtype=np.float32)
    return {"CytoTRACE": scores, "CytoTRACErank": rank,
            "GCS": gcs,
            "Counts": feature_count_per_cell,
            "exprMatrix": census_X,
            "cytoGenes": cytoGenes,
            "gcsGenes": corrs,
            "filteredCells": np.setdiff1d(np.arange(adata.shape[0]),
                                          cells_selected)}
...
#        X = (adata.layers['spliced'] + adata.layers['unspliced'])
    ...
    adata.obs['gcs'] = None
    adata.obs.iloc[cells_selected,
                   adata.obs.columns.get_loc('gcs')] = gcs
    ...
    adata.var.iloc[genes_selected, adata.var.columns.get_loc('cytotrace_corrs')] = cyto_corrs
    return {"CytoTRACE": scores, "CytoTRACErank": rank,
            "GCS": gcs,
            "Counts": counts,
            "exprMatrix": mat2,
...
def visualize(adata, metrics, name='test'):
    ...
    bplot = sns.boxplot(x='clusters', y='cytotrace', data=adata.obs,
                        order=order, width=0.35, ax=ax[1])
...
def compare_cytotrace_ncores(adata, layer='all', cell_count=10, condition='age',
                             solver='nnls', is_normalized=False, ncores=4, batch_cell=2000):
    "Main interface of cytotrace reimplementation used for single dataset with multiple conditions"
    # condition common steps
    ...
    if not is_normalized:
        ...
        # cell x gene
        X = X[:, ~pgenes]
        X = (X.T).multiply(1. / csr_matrix.sum(X.T, axis=0)).tocsr()
        pqcells = np.array((np.isnan((X > 0).sum(axis=0))) | ((X>0).sum(axis=0) <= cell_count))[0]
        X = X[:, ~pqcells]
...
def cytotrace_ncore(adata, layer='all', cell_count=10, solver='nnls',
                    ncores=4, batch_cell=3000,
                    shuffle=3):
    "optimized version"
    proc_time = time()
    if layer == 'all':
    ...
    adata.obs['counts'] = None
    adata.obs.iloc[cells_selected,
                   adata.obs.columns.get_loc('counts')] = counts
...
# Cell
def align_diffrate(adatas, labels, field='condition', type='A',
                   outfield='cytotrace', ax=None):
    "this is used for differentiation rate comparison across samples"
    scores = [adatas[0].obs.loc[:, outfield][adatas[0].obs.loc[:, field]==type],
    ...
    for s, l in zip(scores, labels):
        if outfield in ['latent_time', 'velocity_pseudotime']:
            ...
            if outfield != 'cytotrace':
                ...
                s = np.sort(1-s)
        ax.step(np.concatenate([s, s[[-1]]]),
                np.arange(s.size+1)/s.size, label=l)
        ax.scatter(np.concatenate([s, s[[-1]]]),
                   np.arange(s.size+1)/s.size, s=5)
        if outfield=='cytotrace':
        ...
        ax.set_title(type)
    ax.text(np.percentile(s, 0.6), 0.4, '%s > %s: %.3E'% (labels[0], labels[1],
                                                         pvalg))
    ax.text(np.percentile(s, 0.6), 0.48, '%s < %s: %.3E'% (labels[0], labels[1],
                                                           pvall))
    ax.legend()
...
def plot_multiadata(adatas):
    for a in adatas:
        ...
        scv.pl.velocity_embedding_grid(a, scale=0.2, color='louvain', show=False,
                                       basis='pca', legend_loc='on data', ax=ax[0],
                                       title=a.obs.age[0])
        scv.pl.scatter(a, color='velocity_pseudotime', basis='pca',
                       size=80, color_map='gnuplot',
                       ax=ax[1], show=False, title=a.obs.age[0])
        scv.pl.scatter(a, color='latent_time', basis='pca',
                       size=80, color_map='gnuplot',
                       ax=ax[2], show=False, title=a.obs.age[0])
        scv.pl.scatter(a, color='end_points', basis='pca',
                       size=80, color_map='gnuplot',
                       ax=ax[3], show=False, title=a.obs.age[0])
        scv.pl.scatter(a, color='root_cells', basis='pca',
                       size=80, color_map='gnuplot',
                       ax=ax[4], show=False, title=a.obs.age[0])
